...
class TestCase(utils.test_case.TestCaseAbs):
    ...
    def Prepare(self,parameters, step):
        ...
        parameters.driver_cmd_line_args = [
                ...
                f"problem/cluster/agn_feedback/kinetic_jet_height={self.agn_jet_height.in_units('code_length').v}",
            ]
    ...
    def Analyse(self,parameters):
        ...
        for step in range(1,self.steps+1):
            ...
            jet_density = (agn_kinetic_fraction*self.fixed_power)/(self.efficiency*unyt.c_cgs**2)/( 
                2*np.pi*self.agn_jet_radius**2*self.agn_jet_height)

            jet_velocity = np.sqrt( 2*self.efficiency)*unyt.c_cgs
            ...
            def thermal_feedback(Z,Y,X,time):
                ...
                dE = inside_sphere*time*(self.fixed_power*agn_thermal_fraction/(4./3.*np.pi*self.agn_thermal_radius**3))

                drho = inside_sphere*time*(self.fixed_power/(self.efficiency*unyt.c_cgs**2)*agn_thermal_fraction/(4./3.*np.pi*self.agn_thermal_radius**3))
