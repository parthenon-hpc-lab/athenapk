...
class TestCase(utils.test_case.TestCaseAbs):
    def __init__(self):
        ...
        self.mu = 1/(self.He_mass_fraction*3./4. + (1-self.He_mass_fraction)*2)
        ...
        self.uniform_gas_vel = np.sqrt( self.uniform_gas_ux**2 
                                      + self.uniform_gas_uy**2 
                                      + self.uniform_gas_uz**2 )

        self.uniform_gas_temp = self.mu*self.m_u/self.k_b*self.uniform_gas_pres/self.uniform_gas_rho
    ...
    def Analyse(self,parameters):
        ...
        for step in range(1,self.steps+1):
            ...
            sim_data = np.loadtxt(
                f"{triggering_mode}_triggering.dat")
            ...
            elif triggering_mode == "BOOSTED_BONDI" or triggering_mode == "BOOTH_SCHAYE":
                sim_total_mass = unyt.unyt_array(sim_data[:,3],"code_mass")
                sim_avg_density = unyt.unyt_array(sim_data[:,4],"code_mass/code_length**3")
                sim_avg_velocity = unyt.unyt_array(sim_data[:,5],"code_length/code_time")
                sim_avg_cs = unyt.unyt_array(sim_data[:,6],"code_length/code_time")
            else:
                raise Exception(f"Triggering mode {triggering_mode} not supported in analysis")

            n_times = sim_data.shape[0]

            analytic_density = unyt.unyt_array(np.empty(n_times+1),"code_mass*code_length**-3")
            analytic_pressure = unyt.unyt_array(np.empty(n_times+1),"code_mass/(code_length*code_time**2)")
            analytic_accretion_rate = unyt.unyt_array(np.empty(n_times),"code_mass*code_time**-1")

            analytic_density[0] = self.uniform_gas_rho.in_units("code_mass*code_length**-3")
            analytic_pressure[0] = self.uniform_gas_pres.in_units("code_mass/(code_length*code_time**2)")

            accretion_volume = 4./3.*np.pi*self.accretion_radius**3
            ...
            for i in range(n_times):
                ...
                if triggering_mode == "COLD_GAS":
                    ...
                    accretion_rate = analytic_density[i]*accretion_volume/self.cold_t_acc
                elif triggering_mode == "BOOSTED_BONDI" or triggering_mode == "BOOTH_SCHAYE":
                    ...
                    cs2 = self.adiabatic_index*self.uniform_gas_pres/self.uniform_gas_rho
                    accretion_rate = alpha * \
                        (2 * np.pi *unyt.G_cgs**2 * self.M_smbh**2 * analytic_density[i]) / \
                        (self.uniform_gas_vel**2 + cs2)**(3./2.)
                else:
                    raise Exception(f"Triggering mode {triggering_mode} not supported in analysis")

                accretion_rate_density = accretion_rate/accretion_volume

                analytic_density[i+1] = (analytic_density[i]  - accretion_rate_density*dt).in_units(analytic_density.units)
                analytic_pressure[i+1] = (analytic_pressure[i] \
                    - accretion_rate_density*dt*analytic_pressure[i]/analytic_density[i]).in_units(analytic_pressure.units)
                analytic_accretion_rate[i] = accretion_rate.in_units(analytic_accretion_rate.units)

            #Compare the analytic accretion_rate
            accretion_rate_err = np.abs((analytic_accretion_rate - sim_accretion_rate)/analytic_accretion_rate)

            if np.max(accretion_rate_err) > self.linf_accretion_rate_tol:
                analyze_status = False
                print(f"{triggering_mode} linf_accretion_rate_err {np.max(accretion_rate_err)}"
            ...
            final_energy_density = 1./2.*final_rho*(self.uniform_gas_ux**2
                                                  + self.uniform_gas_uy**2
                                                  + self.uniform_gas_uz**2) \
                        + final_pres/(self.adiabatic_index - 1.)
            ...
            def accretion_mask( Z,Y,X, inner_state, outer_state ):
                ...
                state = inner_state*(r < self.accretion_radius) + outer_state*(r >= self.accretion_radius)
            ...
            initial_analytic_components = {
                    ...
                    "Velocity1":lambda Z,Y,X,time :
                        np.ones_like(Z)*self.uniform_gas_ux.in_units("code_length*code_time**-1").v,
                    "Velocity2":lambda Z,Y,X,time :
                        np.ones_like(Z)*self.uniform_gas_uy.in_units("code_length*code_time**-1").v,
                    "Velocity3":lambda Z,Y,X,time :
                        np.ones_like(Z)*self.uniform_gas_uz.in_units("code_length*code_time**-1").v,
                    "Pressure":lambda Z,Y,X,time :
                        np.ones_like(Z)*self.uniform_gas_pres.in_units("code_mass/(code_length*code_time**2)").v,
                    }
            ...
            final_analytic_components = {
                    "Density":lambda Z,Y,X,time :
                        accretion_mask(Z,Y,X,final_rho,self.uniform_gas_rho).in_units("code_mass/code_length**3").v,
                    "MomentumDensity1":lambda Z,Y,X,time :
                        accretion_mask(Z,Y,X,final_Mx,self.uniform_gas_Mx).in_units("code_mass*code_length**-2*code_time**-1").v,
                    "MomentumDensity2":lambda Z,Y,X,time :
                        accretion_mask(Z,Y,X,final_My,self.uniform_gas_My).in_units("code_mass*code_length**-2*code_time**-1").v,
                    "MomentumDensity3":lambda Z,Y,X,time :
                        accretion_mask(Z,Y,X,final_Mz,self.uniform_gas_Mz).in_units("code_mass*code_length**-2*code_time**-1").v,
                    "TotalEnergyDensity":lambda Z,Y,X,time :
                        accretion_mask(Z,Y,X,final_energy_density,self.uniform_gas_energy_density).in_units("code_mass*code_length**-1*code_time**-2").v,
                    "Velocity1":lambda Z,Y,X,time :
                        np.ones_like(Z)*self.uniform_gas_ux.in_units("code_length*code_time**-1").v,
                    "Velocity2":lambda Z,Y,X,time :
                        np.ones_like(Z)*self.uniform_gas_uy.in_units("code_length*code_time**-1").v,
            ...
            for analytic_components,phdf_file in zip((initial_analytic_components,final_analytic_components),phdf_files):
                analytic_status &= compare_analytic.compare_analytic( phdf_file, analytic_components,
                    err_func=lambda gold,test: compare_analytic.norm_err_func(gold, test, norm_ord=np.inf,relative=True), tol=self.norm_tol)
