...
class TestCase(utils.test_case.TestCaseAbs):
    def __init__(self):
        ...
        self.feedback_magnetic_tower_mass = self.feedback_magnetic_tower_power/( self.efficiency*unyt.c_cgs**2 )
    ...
    def Prepare(self,parameters, step):
        ...
        if feedback_mode == "const_power":
            fixed_power = self.feedback_magnetic_tower_power.in_units('code_mass*code_length**2/code_time**3').v
            fixed_field_rate =  0
            fixed_mass_rate = 0
        else:
            fixed_power = 0
            fixed_field_rate = self.feedback_magnetic_tower_field.in_units('sqrt(code_mass)/sqrt(code_length)/code_time**2').v
            fixed_mass_rate =  self.feedback_magnetic_tower_mass.in_units("code_mass/code_time").v
    ...
    def Analyse(self,parameters):
        ...
        for step in range(1,self.steps+1):
            ...
            if feedback_mode == "const_field":
                ...
                injected_mass = self.feedback_magnetic_tower_power/(self.efficiency*unyt.c_cgs**2)*self.tlim
            else:
                raise Exception(f"Feedback mode {feedback_mode} not supported in analysis")

            rho0_final = injected_mass/(self.magnetic_tower_l_mass_scale**3*np.pi**(3./2.))
            ...
            def density_func(Z,Y,X,rho0):
                ...
                Density = self.uniform_gas_rho + rho0*np.exp( -(R**2 + H**2)/self.magnetic_tower_l_mass_scale**2 )
                return Density
            ...
            def internal_energy_density_func(Z,Y,X,rho0):
                ...
                rho_e = self.uniform_gas_energy_density + 1./(self.adiabatic_index - 1)*(
                    rho0*np.exp( -(R**2 + H**2)/self.magnetic_tower_l_mass_scale**2 )
            ...
            for B_field,rho0,phdf_filename,label in zip(
                ...
                densities_analytic_components = {
                    "Density":lambda Z,Y,X,time : density_func(Z,Y,X,rho0).in_units("code_mass*code_length**-3").v,
