...
- `mixed_plm_wenoz` : `plm` for hydro variables and `wenoz` for magnetic fields
- `limo3` : LimO3 (third order)
- `weno3` : WENO3 (third order)
- `wenoz` : WENO-Z (third order but more accurate than WENO3)

Note, `ppm`, `wenoz`, `mixed_plm_ppm`, and `mixed_plm_wenoz` need at least three ghost
zones (`parthenon/mesh/num_ghost`).
