...
using namespace parthenon;
      ...
      Real AGBWindIntegratedMassDistributionFunction(const Real a, const Real sigma_agb, const Real a_agb, const Real rho_grain){
      ...
      Real AGBWindIntegratedNumberDistributionFunction(const Real a, const Real sigma_agb, const Real a_agb){
        const Real pi = Kokkos::numbers::pi;
        ...
        // printf("t1 = %g t2 = %g t3 = %g n_AGB = % g sigma_agb = % g \n", t1, t2, t3, n_AGB, sigma_agb);
        return n_AGB;

      }

     Real AGBWindIntegratedMassDistribution(const Real a_upper, const Real a_lower, const Real sigma_agb, const Real a_agb, const Real rho_grain){
          return AGBWindIntegratedMassDistributionFunction(a_upper, sigma_agb, a_agb, rho_grain) - AGBWindIntegratedMassDistributionFunction(a_lower, sigma_agb, a_agb, rho_grain);
      }
      Real AGBWindIntegratedNumberDistribution(const Real a_upper, const Real a_lower, const Real sigma_agb, const Real a_agb){
        return AGBWindIntegratedNumberDistributionFunction(a_upper, sigma_agb, a_agb) - AGBWindIntegratedNumberDistributionFunction(a_lower, sigma_agb, a_agb);
      }

Dust::Dust(parthenon::ParameterInput *pin,
    ...
    num_r_bins_(pin->GetOrAddInteger("problem/cluster/dust_history", "num_radial_bins",10)),
    logspace_(pin->GetOrAddBoolean("problem/cluster/dust_history", "log_space",true)),
    min_temp_kelvin_(pin->GetOrAddReal("problem/cluster/dust_history", "min_T_kelvin",0)),
    max_temp_kelvin_(pin->GetOrAddReal("problem/cluster/dust_history", "max_T_kelvin",0)),
    num_temp_bins_(pin->GetOrAddInteger("problem/cluster/dust_history", "num_temperature_bins",10)),
    write_dust_history_to_file_(pin->GetOrAddBoolean("problem/cluster/dust_history", "write_to_file", true)),
    dust_history_filename_(pin->GetOrAddString("problem/cluster/dust_history", "dust_history_filename", "dust_history.dat")),
    ...
    PARTHENON_REQUIRE(grainsize_bins_low_edge_ < grainsize_bins_high_edge_, "grainsize_bins_low_edge_ < grainsize_bins_high_edge_ not satisfied!");
    ...
    mbar_gm1_over_kb = hydro_pkg->Param<Real>("mbar_over_kb") * (pin->GetReal("hydro", "gamma") - 1);
    ...
    grain_midbin_sizes_microm_v_ = std::vector<Real>(num_grainsize_bins_); // will store the midpoints

    // Generate the temperature bin edges (always in log space), store them in temp_bin_edges
    ...
    double dloga = (log_max_grainsize_bin_edge_-log_min_grainsize_bin_edge_)/num_grainsize_bins_;
    // num bin edges is +1 more than number of bins

    // eqn 30 of McKinnon to get bin edges
    for(int i = 0; i<num_grainsize_bins_+1; i++){
        grainsize_bin_edges_microm_v_[i] = std::pow(10, log_min_grainsize_bin_edge_ + (i * dloga));
        if(i>0){
          // eqn 21 of McKinnon to get bin midpoints
          grain_midbin_sizes_microm_v_[i-1] = (grainsize_bin_edges_microm_v_[i] + grainsize_bin_edges_microm_v_[i-1]) / 2.;
        }
    }
    ...
    // Precompute grain masses here so these calculations are done only once instead of within a kernel
    ...
    temp_initial_dust_bin_mass_ratio = pin->GetReal("dust", "carbonaceous_grain_mass_fraction");
    for(int i = 0; i < grain_midbin_sizes_microm_v_.size(); i++){
        grain_radius_code = grain_midbin_sizes_microm_v_[i]*Kokkos::pow(10, -4)*units.cm();
        grain_mass = 4./3.  * Kokkos::numbers::pi_v<double> * Kokkos::pow(grain_radius_code, 3) * carbonaceous_grain_density_; 
    ...
    temp_initial_dust_bin_mass_ratio = pin->GetReal("dust", "silicate_grain_mass_fraction");
    for(int i = 0; i < grain_midbin_sizes_microm_v_.size(); i++){
        grain_radius_code = grain_midbin_sizes_microm_v_[i]*Kokkos::pow(10, -4)*units.cm();
        grain_mass = 4./3.  * Kokkos::numbers::pi_v<double> * Kokkos::pow(grain_radius_code, 3) * silicate_grain_density_; 
        single_grain_midbin_masses_v_.push_back(grain_mass);
        initial_dust_bin_mass_ratios_v_.push_back(temp_initial_dust_bin_mass_ratio);
        num_dust_bins += 1;
    }
...
PARTHENON_REQUIRE(initial_dust_bin_mass_ratios_v_.size() == num_dust_bins, "Bad number of initial_dust_bin_mass_ratios");
  ...
  grain_midbin_sizes_microm_ = ParArray1D<Real>("grain_midbin_sizes_microm", grain_midbin_sizes_microm_v_.size());
  auto host_grain_midbin_sizes_microm = Kokkos::create_mirror_view(grain_midbin_sizes_microm_);
  for (unsigned int i = 0; i < grain_midbin_sizes_microm_v_.size(); i++) {
    host_grain_midbin_sizes_microm(i) = grain_midbin_sizes_microm_v_[i]; // convert from micro meters
  }
  ...
  for (unsigned int i = 0; i < grainsize_bin_edges_microm_v_.size(); i++) {
    host_grainsize_bin_edges_microm(i) = grainsize_bin_edges_microm_v_[i]; // convert from micro meters
  }
  // Copy into device memory
  Kokkos::deep_copy(grainsize_bin_edges_microm_, host_grainsize_bin_edges_microm);


  single_grain_densities_ = ParArray1D<Real>("single_grain_densities", single_grain_densities_v_.size());
  ...
  single_grain_masses_ = ParArray1D<Real>("single_grain_masses", single_grain_midbin_masses_v_.size());
  ...
  initial_dust_bin_mass_ratios_ = ParArray1D<Real>("initial_dust_bin_mass_ratios", initial_dust_bin_mass_ratios_v_.size());
  auto host_initial_dust_bin_mass_ratios = Kokkos::create_mirror_view(initial_dust_bin_mass_ratios_);
  ...
  // printf("Got to end of Kokkos::deep_copy(initial_dust_bin_mass_ratios, host_initial_dust_bin_mass_ratios); \n");

  // For the dust histories output files
  if(write_dust_history_to_file_){
      PARTHENON_REQUIRE(min_radius_ < max_radius_, "min_radius_ < max_radius_ not satisfied for dust histories!");
      PARTHENON_REQUIRE(min_temp_kelvin_ < max_temp_kelvin_, "min_temp_kelvin_ < max_temp_kelvin_ not satisfied for dust histories!");
}
  ...
  if(agb_winds_){
    ...
    for (unsigned int i = 0; i < num_grainsize_bins_; i++) {
      ...
      total_dist_mass_for_norm += AGBWindIntegratedMassDistribution(a_upper, a_lower, sigma_agb, a_agb, rho_d);
    }
  ...
  // Function to measure dust masses and cooling rates in radii and gas T bins and write to file
  void Dust::MeasureAndRecordHistory(parthenon::MeshData<parthenon::Real> *md,const parthenon::SimTime &tm) const {
        if(!this->write_dust_history_to_file_){return;}
        ...
        parthenon::IndexRange ib = md->GetBlockData(0)->GetBoundsI(parthenon::IndexDomain::interior);
        parthenon::IndexRange jb = md->GetBlockData(0)->GetBoundsJ(parthenon::IndexDomain::interior);
        parthenon::IndexRange kb = md->GetBlockData(0)->GetBoundsK(parthenon::IndexDomain::interior);
        ...
        Kokkos::View<double*> device_temp_bin_edges("device_temp_bin_edges",num_temp_bins_+1);
        ...
        Kokkos::View<double***, Kokkos::LayoutRight> reduction_view_dust_cool_rate("reduction_view_dust_cool_rate",num_r_bins_, num_temp_bins_, num_dust_bins);
        Kokkos::Experimental::ScatterView<double***, Kokkos::LayoutRight> scatter_f_dust_cool_rate(reduction_view_dust_cool_rate);
        scatter_f_dust_cool_rate.reset();

        Kokkos::View<double*, Kokkos::LayoutRight> reduction_view_gaseous_cool_rate("reduction_view_gaseous_cool_rate",num_r_bins_);
        Kokkos::Experimental::ScatterView<double*, Kokkos::LayoutRight> scatter_f_gaseous_cool_rate(reduction_view_gaseous_cool_rate);
        scatter_f_gaseous_cool_rate.reset();

        Kokkos::View<double***, Kokkos::LayoutRight> reduction_view_dust_mass("reduction_view_dust_mass",num_r_bins_, num_temp_bins_, num_dust_bins);
        Kokkos::Experimental::ScatterView<double***, Kokkos::LayoutRight> scatter_f_dust_mass(reduction_view_dust_mass);
        ...
        const cooling::TabularCooling &tabular_cooling = hydro_pkg->Param<cooling::TabularCooling>("tabular_cooling");
        ...
        Kokkos::parallel_for(
            ...
            KOKKOS_LAMBDA(const int b, const int dust_i, const int k, const int j, const int i) {
              ...
              // stores both mass and number density for each bin
                    ...
                    auto internal_e =
                                cons_pack(b, IEN, k, j, i) - 0.5 *
                                     (SQR(cons_pack(b, IM1, k, j, i)) + SQR(cons_pack(b, IM2, k, j, i)) +
                                      SQR(cons_pack(b, IM3, k, j, i))) /
                                     rho;
                    
                    if (mhd_enabled) {
                    internal_e -= 0.5 * (SQR(cons_pack(b, IB1, k, j, i)) + SQR(cons_pack(b, IB2, k, j, i)) +
                    ...
                    parthenon::Real r = std::sqrt(SQR(x-meshCentx) + SQR(y-meshCenty) + SQR(z-meshCentz));
                    int rbin_idx = -1;
                    int Tbin_idx = -1;
                    ...
                    for(int Tbin_i = 0; Tbin_i < local_num_temp_bins; Tbin_i ++){
                        if((temperature > device_temp_bin_edges[Tbin_i])&&(temperature <= device_temp_bin_edges[Tbin_i+1])){
                        Tbin_idx = Tbin_i;
                        break;}
                    }
                    ...
                    dust_de_dt = DustDevObj.DwekWernerCooling(temperature, rho, cooling_table_obj.x_H_over_m_h2_, dust_scalar_idx_start, k, j, i, cons,coords,dust_piecewise_mode_int, dust_i);
                    }
                    else if(dust_cooling_mode_ == dust::DustCoolingMode::DWEKWERNER1981_INTEGRATED){
                    dust_de_dt = DustDevObj.DwekWernerCoolingIntegrated(temperature, rho, cooling_table_obj.x_H_over_m_h2_, dust_scalar_idx_start, k, j, i, cons,coords,dust_piecewise_mode_int, dust_i);
                    }
                    ...
                    if((rbin_idx != -1)&&(Tbin_idx != -1)){
                            ...
                            Real block_mass = cons_pack(b, dust_scalar_idx_start + (dust_i*2) + 1 , k, j, i) * coords.CellVolume(k, j, i);
                            f_a_dust_mass(rbin_idx, Tbin_idx, dust_i) += block_mass;
                            // total_dust_de_dt = specific_deDT * mass = specific_deDT * gas_density (because the cooling rate is for the gas) * volume
                            Real total_dust_de_dt = dust_de_dt * cons_pack(b, IDN, k, j, i) * coords.CellVolume(k, j, i);
                    ...
                    if(rbin_idx != -1){
                            ...
                            Real total_gaseous_de_dt = cooling_table_obj.DeDt(internal_e, rho) * cons_pack(b, IDN, k, j, i) * coords.CellVolume(k, j, i);
                ...
                Kokkos::Experimental::contribute(reduction_view_dust_cool_rate,scatter_f_dust_cool_rate);
                // copy reduction view back to host so we can print it
                // Layout rights need to be enforced for the later MPI_reduce
                Kokkos::View<double***, Kokkos::LayoutRight, Kokkos::HostSpace> host_reduction_view_dust_cool_rate = Kokkos::create_mirror_view(reduction_view_dust_cool_rate);
                // Kokkos::View<double**, Kokkos::LayoutRight, Kokkos::HostSpace> host_reduction_view("host_reduction_view", num_r_bins_,num_temp_bins_);
                Kokkos::deep_copy(host_reduction_view_dust_cool_rate, reduction_view_dust_cool_rate);



                Kokkos::Experimental::contribute(reduction_view_gaseous_cool_rate,scatter_f_gaseous_cool_rate);
                // copy reduction view back to host so we can print it
                // Layout rights need to be enforced for the later MPI_reduce
                Kokkos::View<double*, Kokkos::LayoutRight, Kokkos::HostSpace> host_reduction_view_gaseous_cool_rate = Kokkos::create_mirror_view(reduction_view_gaseous_cool_rate);
                // Kokkos::View<double**, Kokkos::LayoutRight, Kokkos::HostSpace> host_reduction_view("host_reduction_view", num_r_bins_,num_temp_bins_);
                Kokkos::deep_copy(host_reduction_view_gaseous_cool_rate, reduction_view_gaseous_cool_rate);
                ...
                // copy reduction view back to host so we can print it
                // Layout rights need to be enforced for the later MPI_reduce
                Kokkos::View<double***, Kokkos::LayoutRight, Kokkos::HostSpace> host_reduction_view_dust_mass = Kokkos::create_mirror_view(reduction_view_dust_mass);
                // Kokkos::View<double**, Kokkos::LayoutRight, Kokkos::HostSpace> host_reduction_view("host_reduction_view", num_r_bins_,num_temp_bins_);
        ...
        if (write_dust_history_to_file_){
          ...
          MPI_Reduce(MPI_IN_PLACE, buf_dm.data(),  num_r_bins_*num_temp_bins_*num_dust_bins, MPI_PARTHENON_REAL, MPI_SUM, 0, MPI_COMM_WORLD);
          MPI_Reduce(MPI_IN_PLACE, buf_dcr.data(), num_r_bins_*num_temp_bins_*num_dust_bins, MPI_PARTHENON_REAL, MPI_SUM, 0, MPI_COMM_WORLD);
          MPI_Reduce(MPI_IN_PLACE, buf_gcr.data(), num_r_bins_, MPI_PARTHENON_REAL, MPI_SUM, 0, MPI_COMM_WORLD);
          ...
          if(my_rank == 0){
            ...
            for(int dust_i = 0 ; dust_i < num_dust_bins; dust_i++){
              for(int dust_j = 0 ; dust_j < 3; dust_j++){ // one for mass view, one for cooling rate view, one for gas cooling rate
                ...
                if(dust_j == 0){
                  ...
                  column_name = "Log Cooling Loss Rate in bin (erg/s)";
                } else{PARTHENON_FAIL("Something gone terribly wrong");}

            if((dust_j == 2)&&(dust_i != 0)){ // for the gas cooling rate, we want to run only once, so skip dust bin iterations
              continue;
            }
            ...
            dust_file << i << ":" << r_bin_edges_[i]/ kpc << "kpc" <<"-->" << r_bin_edges_[i+1]/ kpc << "kpc" << "|" ;
            }
            dust_file << std::endl << "T (K)  Bins::: " ;
            for(int j =0; j<num_temp_bins_; j++){
                dust_file << j << ":" << temp_bin_edges_[j] << "K" <<"-->" << temp_bin_edges_[j+1] << "K" << "|" ;
                }
            if(dust_j != 2){
              ...
              for(int j =0; j<grainsize_bin_edges_microm.size()-1; j++){
                dust_file << j << ":" << grainsize_bin_edges_microm[j] << "(μm)" <<"-->" << grainsize_bin_edges_microm[j+1] << "(μm)" << "|" ;
                }
            }
            dust_file << std::endl;
            ...
            for(int i = 0; i<num_r_bins_; i++){ // radii bins
                ...
                if(dust_j == 0){
                  dust_file << " | " << std::log10(host_reduction_view_dust_mass(i,j, dust_i)/msun_to_code_mass);
                } 
                else if(dust_j == 1){
                  dust_file << " | " << std::log10(-1.0 * host_reduction_view_dust_cool_rate(i,j, dust_i) / (erg_to_code_energy_/seconds_to_code_time_));
                } 
              ...
              for(int i = 0; i<num_r_bins_; i++){ // radii bins
                dust_file << " | " << std::log10(-1.0 * host_reduction_view_gaseous_cool_rate(i) / (erg_to_code_energy_/seconds_to_code_time_));
              }
            }
...
std::vector<double> Dust::get_r_bin_edges() const{
  return this->r_bin_edges_;
};



  Real InterpolateDustReturn(const Real M, const std::vector<Real> m_star, const std::vector<Real> m_dust){
...
Real SalpeterIMF(const Real M, const Real imf_m_lower,  const Real imf_m_upper){
  // returns salpeter IMF at a given M normalised by integration over imf_m_lower - imf_m_upper
// dN/dm_agb = K M^-2.35
...
// printf("[FJJ Test] SalpeterIMF M = %g norm = % g  Phi(M) = %g \n", M, norm, norm * Kokkos::pow(M, -2.35));
return norm * Kokkos::pow(M, -2.35);
} // SalpeterIMF

Real SalpeterIMFMassFractionInRange(const Real M_lower,  const Real M_upper, const Real imf_m_lower,  const Real imf_m_upper){
  // returns integrated salpeter IMF from M_lower to M_upper, normalised by integration over imf_m_lower - imf_m_upper
// dN/dm_agb = K M^-2.35
// N = (K/1.35) * (M_lower^-1.35 - M_upper^-1.35)
// M = (K/0.35) * (M_lower^-0.35 - M_upper^-0.35)
// Real norm = 0.35 / (Kokkos::pow(imf_m_lower, -0.35) - Kokkos::pow(imf_m_upper, -0.35));
return (Kokkos::pow(M_lower, -0.35) - Kokkos::pow(M_upper, -0.35)) / (Kokkos::pow(imf_m_lower, -0.35) - Kokkos::pow(imf_m_upper, -0.35));
} // SalpeterIMF
...
Real AGBReturnIntegrand(const Real M, const Real imf_m_lower,  const Real imf_m_upper,const std::vector<Real> m_star, const std::vector<Real> m_dust){
...
// printf("[FJJ Test] SalpeterIMF M = %g imf_m_lower = % g  imf_m_upper = %g SalpeterIMF(M, imf_m_lower, imf_m_upper) = % g\n", M, imf_m_lower, imf_m_upper, t1);
// printf("[FJJ Test] SalpeterIMF M = %g  InterpolateDustReturn(M, m_star, m_dust) = % g\n", M,  t2);
// printf("[FJJ Test] SalpeterIMF M = %g  StellarMSLifetimeInMyr(M) = % g\n", M,  t3);
return t1 * t2 / t3;
} // AGBReturnIntegrand


Real IntegrateAGBReturnOverIMF(const std::vector<Real> m_star, const std::vector<Real> m_dust){
...
const auto salpeter_imf_mass_fraction_in_agb_range =  SalpeterIMFMassFractionInRange(agb_m_lower,  agb_m_upper, imf_m_lower,  imf_m_upper);
...
for(int i = 0; i < n_integration_steps; i++){
	// Integrate using Simpson's rule. It is very inefficient to recalculate the IMF norm every time, but 
  // this only has to be done once at the start of the simulation, so we don't bother storing it
	...
	Real f_ab_2 = AGBReturnIntegrand( (a+b)/2.,  imf_m_lower, imf_m_upper, m_star, m_dust);
...
// testing - lets see what the trapezoidal rule gives
  ...
  // printf("[FJJ Test] Simpson dresult = %g  Trapezoidal dresult_trapez = % g  perc_diff = %g\n", dresult,  dresult_trapez, perc_diff);
}
return result;
...
void CalculateDustReturnPerSolarMassofStars(parthenon::ParameterInput *pin,
                                            parthenon::StateDescriptor *hydro_pkg){ 
const std::string table_filename = pin->GetString("dust/AGB_data_table", "AGB_table_filename");
...
input.Open(table_filename.c_str(), IOWrapper::FileMode::read);
  ...
  /****************************************
   * Determine m_star and and m_dust_species vectors - this mirrors the data reading in tabular_cooling.cpp
   ****************************************/
  ...
  while (tab_ss.good()) {
    ...
    const auto silicate_indexes_to_sum = pin->GetVector<parthenon::Real>("dust/AGB_data_table", "silicate_column_indexes_to_sum");
    const auto carbonaceous_index = pin->GetInteger("dust/AGB_data_table", "carbonaceous_column_index");
    ...
    printf("AGB Winds Info: dust_return_silicates_per_megayear = %g Msun/ Myr of dust per stellar mass of stars per Myr\n", dust_return_silicates_per_megayear);
    }
    // PARTHENON_FAIL("DEBUG");
    hydro_pkg->AddParam<>("dust_return_carbon_mass_fraction_per_megayear", dust_return_carbon_per_megayear);
      ...
      // Create a device-safe instance of the DustDevObj
        ...
        par_for(
          ...
          par_for(
                ...
                DustFilladotView(temperature, gc_i, gs_i, b, k, j, i,
                                            cons_pack,
                                            DustDevObj,
                                            kb, jb, ib,
                                            dt,
             ...
             par_for(
                ...
                for(int gs_j =0; gs_j < dust_num_grains_sizes; gs_j ++){
                  DustDoUpdateWithadotArray(gc_i, gs_i, gs_j, b, k, j, i,
                                              cons_pack,
                                              DustDevObj,
                                              kb, jb, ib,
                                              dt,
