...
//------------------------------------------------------------------------------
// FJJ TODO
- Should probably solve kappa in terms of N and M/rho instead of N and M, to save passing
about this Mi_renorm_factor
- initial_dust_bin_mass_ratios machinery is duplicated in dust.cpp but again in
cluster.cpp (re-reading carbonaceous_grain_mass_fraction etc) -multiply-used values like
microm_to_code should be added to hydro_pkg instead of being re-calculated
- Add in ability for carbonaceous_column_index to have multiple numbers, like the
silicates
...
//------------------------------------------------------------------------------
// gc_i = index into grain compositions (silicon, carbonates,...) - range is 0-1 if only 2
// grain comps gs_i = index into grain sizes (i.ae. a-bins in the distribution) - range is
// 0-num_size_bins-1 gb_i = gb_i indexes into
// comp0size0,comp0size1,...comp1size0,...compNsizeM . gb_j =  same as gb_i but
// corresponds to the j-indexed fields in McKinnon. These are for bins we are contributing
// TO rather than FROM index_into_Ni = index into **cons** for the number (norm) field for
// the dust composition + grain size index_into_Mi = index into **cons** for the mass
// field for the dust composition + grain size we have e.g. const int index_into_Ni =
// dust_scalar_idx_start + (gb_i*2);   where the 2 is needed because we have grain number
// AND mass fields const int index_into_Mi = index_into_Ni + 1;
...
double StablePowDiff(double xmin, double xmax, double p) {
  ...
  // so xmax^p approx xmid^p + pxmid^(p-1) dx/2 + p(p-1)xmid^(p-2) (dx/2)^2 / 2  +
  // p(p-1)(p-2)xmid^(p-3) (dx/2)^3 / 6 + ... similar for xmin but with alternating signs
  // xmin^p approx xmid^p - pxmid^(p-1) dx/2 + p(p-1)xmid^(p-2) (dx/2)^2 / 2  -
  // p(p-1)(p-2)xmid^(p-3) (dx/2)^3 / 6 + ... get xmax^p - xmin^p approx  p xmid^(p-1) dx
  // + ...
  double x = xmid;
  double hlf_dx = dx / 2.;
...
class Dust {
 public:
  ...
  bool silicate_grains_, carbonaceous_grains_, thermal_sputtering_, coagulation_,
      metal_accretion_, shattering_, dust_on_, agb_winds_;
  Real carbonaceous_grain_density_, silicate_grain_density_, init_dtg_mass_ratio_,
      init_run_stellar_injection_time_, nH_to_ne_, g_cm3_to_code_density_,
      erg_to_code_energy_, seconds_to_code_time_, cm3_to_code_vol_;
  ...
  void WriteAGBInjectionHistory(
      const int num_r_bins_,
      const HostView host_reduction_view_agb_injected_mass_carbonaceous,
      const HostView host_reduction_view_agb_injected_mass_silicates,
      const HostView host_reduction_view_stellar_mass, const HostView_Rbin r_bin_edges,
      parthenon::MeshData<parthenon::Real> *md, const Real dt, const Real t) const;
  ParArray1D<Real> grain_midbin_sizes_microm_, single_grain_densities_,
      single_grain_masses_, initial_dust_bin_mass_ratios_, grainsize_bin_edges_microm_;
...
KOKKOS_INLINE_FUNCTION
int DustGetIndexIntoConsPack_Ni(const int gs_i, const int gc_i,
                                const int num_grains_sizes,
                                const int dust_scalar_idx_start) {
  int idx = 2 * (gc_i * num_grains_sizes) +
            gs_i; // e.g silicon (idx 1) grain size bin 3 (idx 3) with 4 size bins  gives
                  // 2*((1*4) + 3) = 14
  return dust_scalar_idx_start + idx; // offset by length of non-dust cons_pack fields
}
// Index into McKinnon Model bin mass
KOKKOS_INLINE_FUNCTION
int DustGetIndexIntoConsPack_Mi(const int gs_i, const int gc_i,
                                const int num_grains_sizes,
                                const int dust_scalar_idx_start) {
  return DustGetIndexIntoConsPack_Ni(gs_i, gc_i, num_grains_sizes,
                                     dust_scalar_idx_start) +
         1;
}

template <typename View4D>
KOKKOS_INLINE_FUNCTION Real DustGetNumberInBin(const int gs_i, const int gc_i,
                                               const int num_grains_sizes,
                                               const int dust_scalar_idx_start,
                                               const View4D cons, const int k,
                                               const int j, const int i) {
  int Ni_index =
      DustGetIndexIntoConsPack_Ni(gs_i, gc_i, num_grains_sizes, dust_scalar_idx_start);
  Real Ni = cons(Ni_index, k, j, i);
  return Ni;
}

template <typename View4D>
KOKKOS_INLINE_FUNCTION Real DustGetMassInBin(const int gs_i, const int gc_i,
                                             const int num_grains_sizes,
                                             const int dust_scalar_idx_start,
                                             const Real volume, const View4D cons,
                                             const int k, const int j, const int i) {
  int Mi_index =
      DustGetIndexIntoConsPack_Mi(gs_i, gc_i, num_grains_sizes, dust_scalar_idx_start);
  Real Mi = cons(Mi_index, k, j, i) * volume;
  return Mi;
}

// For the linear reconstruction, get the slope in a bin from the number and total mass of
// grains
template <typename View4D>
KOKKOS_INLINE_FUNCTION Real DustGetLinSlopeInBin(
  ...
  Real t2 = (1. / (4. * (aU - aL))) *
            (Kokkos::pow(aU, 4.) - Kokkos::pow(aL, 4.)); // in microM^3
  Real t3 = (Kokkos::pow(aU, 5.) / 5. - (Kokkos::pow(aU, 4.) * aM / 4.)) -
            (Kokkos::pow(aL, 5.) / 5. - (Kokkos::pow(aL, 4.) * aM / 4.)); // in microM^5
  Si = (t1 - (Ni * t2)) / t3; // dn/da in 1/microM^2    (Si has units 1/len^2)

  Real rel_diff = std::abs(t1 - Ni * t2) / std::max(std::abs(t1), std::abs(Ni * t2));
  if (rel_diff < 1e-6) { // Guard against numerical errors if t1 and t2 very close, since
                         // t3 can be v small
    Si = 0.;
  }
...
// For the loglinear reconstruction, calculate mass and number in bin from the prefactor
// and index
KOKKOS_INLINE_FUNCTION
void GetMiNifromKappaBetaForNR(Real &Ni, Real &Mi, const Real kappa, const Real beta,
  ...
  Ni =
      (beta / kap_p_one) *
      StablePowDiff(
          aL, aU, kap_p_one); //(Kokkos::pow(aU, kap_p_one) - Kokkos::pow(aL, kap_p_one));
  Mi = (4. * rho_d * Kokkos::numbers::pi * beta / (3. * kap_p_four)) *
       StablePowDiff(
           aL, aU,
           kap_p_four); //(Kokkos::pow(aU, kap_p_four) - Kokkos::pow(aL, kap_p_four));
}

// For the loglinear reconstruction, use Newton-Raphson to estimate the prefactor and
// index for the loglinear function in the bin
KOKKOS_INLINE_FUNCTION
void DustGetLogLinKappaBetaInBin(Real &kappa_i, Real &beta_i, const Real Ni,
                                 ...
                                 const Real Mi_renorm_factor, Real tol = 0.0001) {

  int n_iter = 0;
  int n_iter_max = 50;
  Real reconstructedMiNi_tol = 0.0001;
  Real max_kappa =
      45; // For range of amin and amx 1e-6 - 1 this should give safe reconstruction
  ...
  if (Ni < 1.e-100 || Mi < 1.e-100) {
    // printf("[DUST WARNING!] Ni=%e or Mi=%e very small so kappa_i and beta_i set to 0.
  ...
  Real aM = grain_midbin_sizes_microm[gs_i];      // Bin mid     in MicroM
  Real rho_d =
      single_grain_densities[gc_i] /
      Mi_renorm_factor; // code_mass / code_len^3 (then possibly into renormed form)
  rho_d = rho_d / Kokkos::pow(code_to_microm,
                              3.); // code_mass / microM^3 (in possibly renormed form)
  ...
  // To successfully exit the NR, we must either hit the max number of allowed iterations,
  // or determine kappa to within a given error AND with that kappa reconstruct Ni and Mi
  // which match the known values to within a given tolerance

  int accepted_Mi_and_Ni = 0;
  while (accepted_Mi_and_Ni == 0) {
    // if kappa is determined to within tol, but Mi and Ni are not matched to within
    // reconstructedMiNi_tol, then we lower the tol on kappa and re-do NR with the
    // previously best kappa as the initial guess
    ...
    while (accepted_kappa == 0) {
      ...
      // Clamp Kappa. If kappa exceeds these we can use the hybrid delta function scheme
      // if the user has activated it.
      if (kappa > max_kappa) {
        kappa = max_kappa;
      }
      ...
      Real f_kap_t1_n = 4. * rho_d * Kokkos::numbers::pi * Ni *
                        (kap_p_one)*StablePowDiff(
                            aL, aU, kap_p_four); //(aU_p_kap_p_four - aL_p_kap_p_four);
      ...
      Real f_dash_kap_t3_d =
          StablePowDiff(aL, aU, kap_p_four); // aU_p_kap_p_four - aL_p_kap_p_four;
      Real f_dash_kap_t3 = f_dash_kap_t3_n / f_dash_kap_t3_d;

      Real f_dash_kap_t4_n = (aU_p_kap_p_one * log_aU) - (aL_p_kap_p_one * log_aL);
      Real f_dash_kap_t4_d =
          StablePowDiff(aL, aU, kap_p_one); // aU_p_kap_p_one - aL_p_kap_p_one;
      ...
      // FJJ DEBUGGING CODE
      // if(f_kap != f_kap){
      // printf("[FJJ DEBUG][DUST WARNING!] n_iter = %d Ni = %e Mi = %e kappa = %e
      // f_kap_t1_n=%e f_kap_t1_d=%e f_kap_t1=%e f_kap=%e\n \n", n_iter, Ni, Mi, kappa,
      //       f_kap_t1_n, f_kap_t1_d, f_kap_t1, f_kap);
      // }
      // if (f_dash_kap != f_dash_kap){
      //     printf("[FJJ DEBUG][DUST WARNING!] f_dash_kap is NaN! n_iter = %d kappa = %e
      //     f_dash_kap_t1=%e f_dash_kap_t2=%e f_dash_kap_t3_n=%e f_dash_kap_t3_d=%e "
      //           "f_dash_kap_t3=%e f_dash_kap_t4_n=%e f_dash_kap_t4_d=%e
      //           f_dash_kap_t4=%e " "f_dash_kap=%e\n \n", n_iter, kappa, f_dash_kap_t1,
      //           f_dash_kap_t2, f_dash_kap_t3_n, f_dash_kap_t3_d, f_dash_kap_t3,
      //           f_dash_kap_t4_n, f_dash_kap_t4_d, f_dash_kap_t4, f_dash_kap);
      // }

      // if (!isfinite(f_dash_kap) || abs(f_dash_kap) < 1e-100){
      //     printf("[FJJ DEBUG][DUST WARNING!] f_dash_kap is tiny or infinite! n_iter =
      //     %d Ni = %e Mi = %e kappa = %e f_kap_t1_n=%e f_kap_t1_d=%e f_kap_t1=%e
      //     f_kap=%e\n \n", n_iter, Ni, Mi, kappa,
      //       f_kap_t1_n, f_kap_t1_d, f_kap_t1, f_kap);
      // }
      ...
      if (kappa_error < tol) {
        ...
        // printf("[DUST WARNING!] Reached n_iter_max with kappa_error = %e tol = %e kappa
    ...
    // reconstruct Ni and Mi with the provisionally accepted kappa, compare to the true
    // values
    kap_p_four = kappa + 4.;
    kap_p_one = kappa + 1.;
    // aU_p_kap_p_four = Kokkos::pow(aU, kap_p_four);
    ...
    if (Kokkos::abs(Mi_reconstructed - Mi) / Mi < reconstructedMiNi_tol &&
      ...
      accepted_Mi_and_Ni = 1;
      // printf("[FJJ DEBUG ] - accepted Newton Raphson kappa after n_iter = %d beta = %g
      // kappa = %g Kokkos::abs(Mi_reconstructed-Mi)/Mi = %g
      // Kokkos::abs(Ni_reconstructed-Ni)/Ni = %g  Mi_reconstructed = %g Mi = %g
      // Ni_reconstructed = %g Ni = %g \n", n_iter,beta,kappa,
      // Kokkos::abs(Mi_reconstructed-Mi)/Mi, Kokkos::abs(Ni_reconstructed-Ni)/Ni,
      // Mi_reconstructed , Mi ,  Ni_reconstructed , Ni );
    } else if (n_iter == n_iter_max) {
      // printf("[DUST WARNING!] Reached n_iter_max at kappa = %g with
      // Kokkos::abs(Mi_reconstructed-Mi)/Mi = %g Kokkos::abs(Ni_reconstructed-Ni)/Ni = %g
      // Ni = %e Mi = %e size bin gs_i=%d \n",kappa, Kokkos::abs(Mi_reconstructed-Mi)/Mi,
      // Kokkos::abs(Ni_reconstructed-Ni)/Ni, Ni, Mi, gs_i);
      accepted_Mi_and_Ni = 1;
    } else {
      tol *= 0.1;
    }
  ...
  if (n_iter != n_iter_max) {
    ...
    PARTHENON_REQUIRE(Kokkos::abs(Mi_reconstructed - Mi) / Mi <
                          reconstructedMiNi_tol * 1.05,
                      "[FJJ DEBUG] Kokkos::abs(Mi_reconstructed-Mi)/Mi < "
                      "reconstructedMiNi_tol"); // Allow a slight tolerance
    PARTHENON_REQUIRE(Kokkos::abs(Ni_reconstructed - Ni) / Ni <
                          reconstructedMiNi_tol * 1.05,
                      "[FJJ DEBUG] Kokkos::abs(Ni_reconstructed-Ni)/Ni < "
                      "reconstructedMiNi_tol"); // Allow a slight tolerance
  }
...
template <typename View4D>
KOKKOS_INLINE_FUNCTION Real DustGetNumberDensityInBinFromLinSlopeAndMass(
    const Real Si, const int index_into_Mi, const int index_into_Ni,
...
MRNGrainSizeDist(const Real total_dust_mass, const int index_into_Mi,
  ...
  Real rho_d = single_grain_densities[gc_i];       // code_mass/code_len^3
  rho_d = rho_d / Kokkos::pow(code_to_microm, 3.); // code_mass / microM**3
  // total dust mass = integral of 4pi/3 * rho_grain * a^3 * Ddn/da   da   where D is the
  // normalisation constant For MRN dn/da = Da^-4.5
  Real D = total_dust_mass / ((8. * Kokkos::numbers::pi * rho_d / 3.) *
                              ((1 / Kokkos::sqrt(a_min)) - (1 / Kokkos::sqrt(a_max))));
  Real bin_a_min = grainsize_bin_edges_microm[gs_i];
  Real bin_a_max = grainsize_bin_edges_microm[gs_i + 1];

  cons(index_into_Mi, k, j, i) =
      D * (8. * Kokkos::numbers::pi * rho_d / 3.) *
      ((1. / Kokkos::sqrt(bin_a_min)) - (1. / Kokkos::sqrt(bin_a_max))) / volume;
  if (std::abs(cons(index_into_Mi, k, j, i)) < 1e-80) {
    printf("cons(index_into_Mi, k, j, i)=%e A=%e B=%e C=%e index_into_Mi=%d "
           "index_into_Ni=%d i=%d j=%d k=%d \n",
           cons(index_into_Mi, k, j, i),
           D * (8. * Kokkos::numbers::pi * rho_d / 3.) *
               ((1. / Kokkos::sqrt(bin_a_min)) - (1. / Kokkos::sqrt(bin_a_max))) / volume,
           (1. / Kokkos::sqrt(bin_a_min)), (1. / Kokkos::sqrt(bin_a_max)), index_into_Mi,
           index_into_Ni, i, j, k);
  }
  cons(index_into_Ni, k, j, i) =
      D * (1 / 3.5) * (Kokkos::pow(bin_a_min, -3.5) - Kokkos::pow(bin_a_max, -3.5)) /
      volume;
  if (std::abs(cons(index_into_Mi, k, j, i)) < 1e-80) {
    printf("after Ni cons(index_into_Mi, k, j, i)=%e A=%e B=%e C=%e index_into_Mi=%d "
           "index_into_Ni=%d i=%d j=%d k=%d \n",
           cons(index_into_Mi, k, j, i),
           D * (8. * Kokkos::numbers::pi * rho_d / 3.) *
               ((1. / Kokkos::sqrt(bin_a_min)) - (1. / Kokkos::sqrt(bin_a_max))) / volume,
  ...
  //   printf("after Ni cons(index_into_Mi, k, j, i)=%e bin_a_min=%e bin_a_max=%e
  //   volume=%e D=%e total_dust_mass=%e\n", cons(index_into_Mi, k, j, i), bin_a_min,
  //   bin_a_max, volume, D, total_dust_mass ); printf("after Ni A=%e B=%e C=%e
  //   index_into_Mi=%d index_into_Ni=%d \n", (8. * Kokkos::numbers::pi * rho_d / 3.) *
  //   ((1./Kokkos::sqrt(bin_a_min)) -
  //   (1./Kokkos::sqrt(bin_a_max))),(1./Kokkos::sqrt(bin_a_min)),(1./Kokkos::sqrt(bin_a_max)),
...
InverseMRNGrainSizeDist(const Real total_dust_mass, const int index_into_Mi,
  ...
  rho_d = rho_d / Kokkos::pow(code_to_microm, 3.); // code_mass / microM**3
  // total dust mass = integral of 4pi/3 * rho_grain * a^3 * Ddn/da   da   where D is the
  // normalisation constant For inverse MRN dn/da = Da^4.5
  Real D = total_dust_mass / ((4. * Kokkos::numbers::pi * rho_d / (3. * 8.5)) *
                              (Kokkos::pow(a_max, 8.5) - Kokkos::pow(a_min, 8.5)));
  Real bin_a_min = grainsize_bin_edges_microm[gs_i];
  Real bin_a_max = grainsize_bin_edges_microm[gs_i + 1];
  cons(index_into_Mi, k, j, i) =
      D *
      ((4. * Kokkos::numbers::pi * rho_d / (3. * 8.5)) *
       (Kokkos::pow(bin_a_max, 8.5) - Kokkos::pow(bin_a_min, 8.5))) /
      volume;
  cons(index_into_Ni, k, j, i) =
      D * (1 / 5.5) * (Kokkos::pow(bin_a_max, 5.5) - Kokkos::pow(bin_a_min, 5.5)) /
      volume;
...
FlatGrainSizeDist(const Real total_dust_mass, const int index_into_Mi,
  ...
  rho_d = rho_d / Kokkos::pow(code_to_microm, 3.); // code_mass / microM**3
  // total dust mass = integral of 4pi/3 * rho_grain * a^3 * Ddn/da   da   where D is the
  // normalisation constant For flat dist dn/da = D (const)
  Real D =
      total_dust_mass / ((4. * Kokkos::numbers::pi * rho_d / 3.) *
                         (Kokkos::pow(a_max, 4.) / 4. - Kokkos::pow(a_min, 4.) / 4.));
  Real bin_a_min = grainsize_bin_edges_microm[gs_i];
  Real bin_a_max = grainsize_bin_edges_microm[gs_i + 1];
  cons(index_into_Mi, k, j, i) =
      D *
      ((4. * Kokkos::numbers::pi * rho_d / 3.) *
       (Kokkos::pow(bin_a_max, 4.) / 4. - Kokkos::pow(bin_a_min, 4.) / 4.)) /
      volume;
...
KOKKOS_INLINE_FUNCTION void FlatGrainSizeDistInRange(
  ...
  // For testing, put all mass/grains into a single size bin specified by
  // flat_graindist_in_range_amin
  Real bin_a_min = grainsize_bin_edges_microm[gs_i];
  Real bin_a_max = grainsize_bin_edges_microm[gs_i + 1];
  ...
  // total dust mass = integral of 4pi/3 * rho_grain * a^3 * Ddn/da   da   where D is the
  // normalisation constant For flat dist dn/da = D (const)
  Real D = total_dust_mass / ((4. * Kokkos::numbers::pi * rho_d / 3.) *
                              (Kokkos::pow(flat_graindist_in_range_amax, 4.) / 4. -
                               Kokkos::pow(flat_graindist_in_range_amin, 4.) / 4.));
  Real xmin = std::max(bin_a_min, flat_graindist_in_range_amin);
  Real xmax = std::min(bin_a_max, flat_graindist_in_range_amax);
  cons(index_into_Mi, k, j, i) =
      D *
      ((4. * Kokkos::numbers::pi * rho_d / 3.) *
       (Kokkos::pow(xmax, 4.) / 4. - Kokkos::pow(xmin, 4.) / 4.)) /
      volume;
...
struct DustDevice {
  ...
  Real DwekWernerGrainCoolingIntegralsLinSlopeHelper(
    ...
    if (Integral_Number == 0) {
      t1_a = (Ni / (3. * bin_width));
      t1_b = StablePowDiff(xmin, xmax,
                           3.); //  Kokkos::pow(x_max, 3.) - Kokkos::pow(x_min, 3.);
      t1 = t1_a * t1_b;
      ...
      result = -1. * result * dwek_werner_coeff_a_code_units *
               Kokkos::pow(temperature, 3.0 / 2.0) * ne_over_V;
      ...
      result = -1. * result * dwek_werner_coeff_b_code_units *
               Kokkos::pow(temperature, 0.88) * ne_over_V;
    ...
    if (result != result) {
      printf("[FJJ DEBUG] Dwek Result is NaN! "
             ...
             Integral_Number, Ni, Si, xmin, xmax, bin_a_mid, temperature, ne_over_V, t1_a,
             t1_b, t2_a, t2_b);
    }
    return result;
  ...
  // Helper function for calculating the cooling rates for the Log-Linear reconstruction
  // method
  KOKKOS_INLINE_FUNCTION
  Real DwekWernerGrainCoolingIntegralsLogLinSlopeHelper(const Real kappa, const Real beta,
    ...
    if (std::abs(kappa) < 1000) {
      if (Integral_Number == 0) {
        result =
            (beta / SafeDenom(kappa + 3.)) *
            StablePowDiff(xmin, xmax, kappa + 3.); //   (Kokkos::pow(x_max, kappa + 3.) -
                                                   //   Kokkos::pow(x_min, kappa + 3.));
        result = -1. * result * dwek_werner_coeff_a_code_units *
                 Kokkos::pow(temperature, 3.0 / 2.0) * ne_over_V;
      } else if (Integral_Number == 1) {
        result =
            (beta / SafeDenom(kappa + 3.41)) * StablePowDiff(xmin, xmax, kappa + 3.41);
        result = -1. * result * dwek_werner_coeff_b_code_units *
                 Kokkos::pow(temperature, 0.88) * ne_over_V;
      } else if (Integral_Number == 2) {
      ...
      if (result != result) {
        printf("[FJJ DEBUG] Dwek Result is NaN! "
               ...
               Integral_Number, xmin, xmax, kappa);
      }
      return result;
      ...
      // the following gives per grain cooling rate * num_density of grains * num_density
      // of gas
      if (Integral_Number == 0) {
  ...
  // This function splits the cooling integral up depending on which Dwek-Werner regimes
  // are intersected, then handles the calls to the DW functions, resulting in a single
  // final dust cooling rate
  KOKKOS_INLINE_FUNCTION
  Real ComputeDwekWernerGrainCooling(
    ...
    // gb_i indexes into comp0size0,comp0size1,...comp1size0,...compNsizeM.  gs_i is index
    // into grain sizes
    ...
    if (integrated_rates == 0) {
      ...
      if (chi >= chi_high_regime) {
        ...
        dust_de_dt_this_grain_bin = dwek_werner_coeff_c_code_units *
                                    Kokkos::pow(grain_midbin_sizes_microm[gs_i], 3);
      } else {
        printf("chi = %g gs_i = %d temperature  %g grain_midbin_sizes_microm[gs_i] = %g "
               "Kokkos::pow(grain_midbin_sizes_microm[gs_i],"
               " 2.0/3.0) = %g dwek_werner_regime_coeff = %g \n",
               chi, gs_i, temperature, grain_midbin_sizes_microm[gs_i],
      ...
      // use the fact that chi ~ a^2/3 is monotonically increasing for fixed T for the
      // following spliting of integral
      ...
      Real a_boundary_low =
          Kokkos::pow(chi_low_regime * temperature / dwek_werner_regime_coeff, 3. / 2.);
      Real a_boundary_high =
          Kokkos::pow(chi_high_regime * temperature / dwek_werner_regime_coeff, 3. / 2.);
      ...
      if (dust_piecewise_mode_int == 1) { // Linear interpolation
        ...
        // Real check_consistent_slope = dust::DustGetLinSlopeInBin(index_into_Mi,
        // index_into_Ni, code_to_microm, gs_i, gc_i, volume, cons, cons_k, cons_j,
        // cons_i, grainsize_bin_edges_microm, grain_midbin_sizes_microm,
        // single_grain_densities); Real check_consistent_number = volume *
        // dust::DustGetNumberDensityInBinFromLinSlopeAndMass(check_consistent_slope,
        // index_into_Mi, index_into_Ni, code_to_microm, gs_i, gc_i,  volume, cons,
        // cons_k, cons_j, cons_i, grainsize_bin_edges_microm, grain_midbin_sizes_microm,
        // single_grain_densities);
        // // printf("std::abs(check_consistent_number-Ni)/Ni = % g
        // check_consistent_number = % g Ni = %g \n",
        // std::abs(check_consistent_number-Ni)/Ni, check_consistent_number, Ni); if(
        // (std::abs(check_consistent_number-Ni)/Ni > 0.1) && (Ni>0) &&
        // (check_consistent_number>0) && (cons(IDN, cons_k, cons_j, cons_i) * volume >
        // 1e-50) ){
        //   printf("[FJJ DEBUG] check_consistent_slope = %g \n", check_consistent_slope);
        //   printf("Ni = %g \n", Ni);
        //   printf("cons(index_into_Mi, k, j, i) = %g \n", cons(index_into_Mi, cons_k,
        //   cons_j, cons_i)); printf("check_consistent_number in dwek werner cooling =
        //   %g, Ni = %g  check_consistent_slope = %g cons(index_into_Mi, k, j, i) = %g
        //   \n", check_consistent_number, Ni, check_consistent_slope, cons(index_into_Mi,
        //   cons_k, cons_j, cons_i)); PARTHENON_FAIL("(check_consistent_number-Ni)/Ni >
        //   0.01");
        // }

      } else if (dust_piecewise_mode_int == 2) { // LogLinear interpolation{
        const Real Mi_renorm_factor = 1; // For cooling rates, we do not renorm Mi and Ni
                                         // to low oom to reduce numerical errors.
        ...
        dust::DustGetLogLinKappaBetaInBin(
            kappa_i, beta_i, Ni, Mi, code_to_microm, gs_i, gc_i,
  ...
  Real DwekWernerCoolingIntegrated(const Real temperature, const Real gas_rho,
    ...
    bool print_perc_difference_to_non_integrated =
        false; // FJJ Set false for production runs.
    if (print_perc_difference_to_non_integrated) {
      Real non_integrated_rate = DwekWernerCooling(
          temperature, gas_rho, x_H_over_m_h2_, dust_scalar_idx_start, cons_k, cons_j,
          cons_i, cons, coords, dust_piecewise_mode_int, single_dust_bin, 0);
      Real integrated_rate =
          DwekWernerCooling(temperature, gas_rho, x_H_over_m_h2_, dust_scalar_idx_start,
                            cons_k, cons_j, cons_i, cons, coords, dust_piecewise_mode_int,
                            single_dust_bin, integrated_rates);

      if (std::abs(100. * (integrated_rate - non_integrated_rate) / integrated_rate) >
          50.) {
        printf("integrated_rate = %g non_integrated_rate = %g perc diff = %.3g\n",
               integrated_rate, non_integrated_rate,
               100. * (integrated_rate - non_integrated_rate) / integrated_rate);
      }
    }
    // Real DEBUG_nonintegrated_dust_de_dt = DwekWernerCooling(temperature, gas_rho,
    // x_H_over_m_h2_, dust_scalar_idx_start, cons_k, cons_j, cons_i, cons, coords,
    // dust_piecewise_mode_int, 0); Real dust_de_dt = DwekWernerCooling(temperature,
    // gas_rho, x_H_over_m_h2_ , dust_scalar_idx_start, cons_k, cons_j, cons_i,  cons,
    // coords, dust_piecewise_mode_int, single_dust_bin, integrated_rates);
    // printf("DwekWernerCoolingIntegrated
    // 100*(DEBUG_nonintegrated_dust_de_dt-dust_de_dt)/dust_de_dt = % g \n",
  ...
  void SetupDustDevice(parthenon::StateDescriptor *hydro_pkg, MeshBlock *pmb) {
    ...
    if (dust_sputtering_on) {
      if (parthenon::Globals::my_rank == 0) {
        printf("Dust: Will do Sputtering \n");
      }
      sputtering = 1;
    ...
    if (dust_metal_accretion_on) {
      if (parthenon::Globals::my_rank == 0) {
        printf("Dust: Will do Metal Accretion \n");
      }
    ...
    if (agb_winds_on == 1) {
      ...
      // Kokkos::create_mirror_view(agb_normalised_carbonaceous_number_distribution_array);
      // Kokkos::deep_copy(host_AGB_normalised_carbonaceous_number_distribution_array,
      // agb_normalised_carbonaceous_number_distribution_array);
    } // if(AGB_winds_on == 1)
  ...
  void SetupDustForEvolutionandCoolingKernel(MeshData<Real> *md) {
    ...
    // Create device views for storing the updated mass and grain number. These will be
    // contributed to from many threads.
    Mj_new = Kokkos::View<Real ******>("dust_Mj_new", num_grain_compositions,
                                       dust_num_grains_sizes, cons_pack.GetDim(5),
                                       1 + kb.e - kb.s, 1 + jb.e - jb.s, 1 + ib.e - ib.s);
    Kokkos::deep_copy(Mj_new, 0.);
    Nj_new = Kokkos::View<Real ******>("dust_Nj_new", num_grain_compositions,
                                       dust_num_grains_sizes, cons_pack.GetDim(5),
                                       1 + kb.e - kb.s, 1 + jb.e - jb.s, 1 + ib.e - ib.s);
    Kokkos::deep_copy(Nj_new, 0.);
    a_dot_view = Kokkos::View<Real ******>(
        "dust_a_dots", num_grain_compositions, dust_num_grains_sizes, cons_pack.GetDim(5),
        1 + kb.e - kb.s, 1 + jb.e - jb.s, 1 + ib.e - ib.s);
    ...
    if (dust_time_integrator_int == 2) {
      heun_state_0 = Kokkos::View<Real *****>("heun_state_0", 2 * dust_num_grains_sizes,
                                              cons_pack.GetDim(5), 1 + kb.e - kb.s,
                                              1 + jb.e - jb.s, 1 + ib.e - ib.s);
      heun_state_1 = Kokkos::View<Real *****>("heun_state_1", 2 * dust_num_grains_sizes,
                                              cons_pack.GetDim(5), 1 + kb.e - kb.s,
                                              1 + jb.e - jb.s, 1 + ib.e - ib.s);
      heun_state_2 = Kokkos::View<Real *****>("heun_state_2", 2 * dust_num_grains_sizes,
                                              cons_pack.GetDim(5), 1 + kb.e - kb.s,
                                              1 + jb.e - jb.s, 1 + ib.e - ib.s);
...
Real PrugnielSimienStellarRhoProfile(const Real r, const Real n, const Real Re,
  ...
  // printf("r = %e, n  = %e, Re  = %e, rho_0 = %e, stellar_density_profile_r_low  = %e,
  // stellar_density_profile_r_up  = %e \n", r , n  , Re  , rho_0 ,
  // stellar_density_profile_r_low  , stellar_density_profile_r_up);
  if (r < stellar_density_profile_r_low || r > stellar_density_profile_r_up) {
    return 0.;
  }
  const Real b =
      2 * n - 1. / 3. + 0.009876 / n; // approx given in 10.1111/j.1365-2966.2005.09269.x
  PARTHENON_REQUIRE(n < 10. && n > 0.6,
                    "Need n < 10. && n > 0.6 in PrugnielSimienStellarRhoProfile");
  const Real p = 1. - 0.6097 / n + 0.05563 / (n * n);
  Real rho_r =
      rho_0 * Kokkos::pow(r / Re, -p) * Kokkos::exp(-b * Kokkos::pow(r / Re, 1. / n));
  return rho_r;
}
...
inline // inline here so all similar functions can be together
    Real
    PrugnielSimienStellardM(const Real r, const Real n, const Real Re, const Real rho_0) {
  return 4. * Kokkos::numbers::pi * r * r *
         PrugnielSimienStellarRhoProfile(r, n, Re, rho_0);
}

inline // inline here so all similar functions can be together
    Real
    GetPrugnielSimienNorm(const Real Mstar_cent, const Real R_low, const Real R_up,
                          const Real n, const Real Re) {
  // printf("In Norm Mstar_cent = %e, n  = %e, Re  = %e,  stellar_density_profile_r_low  =
  // %e, stellar_density_profile_r_up  = %e \n", Mstar_cent , n  , Re  ,  R_low  , R_up);
  const int n_integration_steps = 100000;
  const Real dR = (R_up - R_low) / n_integration_steps;
  Real result = 0.;
  // Integrate using Simpson's rule. There is an an analytic form
  // in 10.1111/j.1365-2966.2005.09269.x But this is simpler and must only be done once
...
Real StellarDensityAtRadiusPowerLaw(const Real r, const Real gamma_star,
  ...
  // Returns stellar density at a radius based on logarithmic profile, normalised to total
  // BCG stellar mass rho_star = D * r^gamma, D is a norm const dMstar(r) = 4pi  *
  // rho_star * r^2 dr = 4pi*D r^(2 + gamma) dr
  if (r < stellar_density_profile_r_low || r > stellar_density_profile_r_up) {
    return 0.;
  }
  Real norm;
  ...
  if (!(gamma_star > -3.001 &&
        gamma_star < -2.999)) { // Guard against gamma_star = -3 case
    ...
    Real norm_d = 4. * Kokkos::numbers::pi *
                  (Kokkos::pow(stellar_density_profile_r_up, gthr) -
                   Kokkos::pow(stellar_density_profile_r_low, gthr));
    norm = norm_n / norm_d;
  } else {
    norm = stellar_mass_cent * 1. /
           (4. * Kokkos::numbers::pi *
            Kokkos::log(stellar_density_profile_r_up / stellar_density_profile_r_low));
  }
  // printf("r = %e norm = %e gamma_star = %e stellar_density_profile_r_low =%e
  // stellar_density_profile_r_up=%e stellar_mass_cent=%e\n", r, norm, gamma_star,
  // stellar_density_profile_r_low, stellar_density_profile_r_up, stellar_mass_cent);
...
// As in McKinnon scheme, do slope-limiting for the linear reconstruction scheme if the
// slope is steep enough to predict negative dust mass at any grain size. To do this we
// enforce mass conservation at the cost of changing the number of grains in a bin. We can
// set fail_on_bad_slope == 1 if we want to fail if slope limiting is required - e.g. if
// we are somewhere in the code where the slope should never need to be limited
KOKKOS_INLINE_FUNCTION
void DustSlopeLimitingLinSlope(const int index_into_Mi, const int index_into_Ni,
  ...
  if (N_at_edge_min < -1. * Ni * tol ||
      N_at_edge_max < -1. * Ni * tol) { // if the values are tiny (effectively zero) at
                                        // edge, don't bother running
    Real a_edge_bad;
    Real N_bad;
    if (N_at_edge_min < -1. * Ni * tol) {
    ...
    if (fail_on_bad_slope == 1 &&
        std::abs(N_bad / Ni) > tol) { // The edge was not set sufficiently close to zero
      printf(
          "[FJJ DEBUG] Slope-limiting FAILURE!: gas_mass_i = %g  Mi = %g Si = %g  Ni = "
          "%g "
    ...
    Real t1_b =
        (Kokkos::pow(bin_a_max, 5.) / 5.) - (bin_a_mid * Kokkos::pow(bin_a_max, 4.) / 4.);
    Real t2_a = (bin_a_mid - a_edge_bad) * Kokkos::pow(bin_a_min, 4.) / 4.;
    Real t2_b =
        (Kokkos::pow(bin_a_min, 5.) / 5.) - (bin_a_mid * Kokkos::pow(bin_a_min, 4.) / 4.);
    Real denom = (t1_a + t1_b) - (t2_a + t2_b);
    denom = denom * 4. * Kokkos::numbers::pi * rho_d / 3.;
    Real Si_new =
        Mi / denom; // Enforce that the mass is constant throughought slope limiting, and
                    // at the bad edge the dist -> 0, to obtain Si_new
    Real Ni_new = -1. * Si_new * (a_edge_bad - bin_a_mid) *
                  (bin_a_max - bin_a_min); // This is the condition that at the previously
                                           // bad edge, the distribution is set to zero

    // printf("New Mi 3 = %g old = %g\n", Ni_new / volume , cons(index_into_Ni, cons_k,
    // cons_j, cons_i));
    cons(index_into_Ni, cons_k, cons_j, cons_i) = Ni_new / volume;

    printf("[FJJ DEBUG] Slope-limiting performed: gas_mass_i = %g  Mi = %g Si = %g "
           "Si_new = %g Ni = %g "
           "Ni_new = %g perc diff = %.3g N_at_edge_min = %g N_at_edge_max = %g "
           "index_into_Mi =%d index_into_Ni =%d \n",
           gas_mass_i, Mi, Si, Si_new, Ni, Ni_new, 100 * (Ni - Ni_new) / Ni,
           N_at_edge_min, N_at_edge_max, index_into_Mi, index_into_Ni);
...
KOKKOS_INLINE_FUNCTION
Real DustCalculateAdotPerBin(const Real temperature, const Real rho,
                             const DustDevice &DustDevObj, Real &adot_sputter,
                             Real &adot_accretion, Real &adot_total) {
  // PGrete - I do the following for cleanliness, but I could just use the attributes
  // directly here and in other kernels. Let me know if it is better to do that.
  ...
  if (sputtering == 1) {
    Real sput_prefac =
        -f_sput * (3.2 * 1e-18 * Kokkos::pow(cm_to_microm, 4.) / seconds_to_code);
  ...
  if (gas_phase_accretion == 1) {
    ...
    n_H /= code_to_cm3;
    adot_accretion = z_on_zsun * (n_H / 1000.) * Kokkos::sqrt(temperature / 10.) *
...
// Implement eqn 37 of McKinnon+18 to calculate the mass to be contributed to a given
// single bin by another single bin during the update
KOKKOS_INLINE_FUNCTION
Real DustGetContributedMassLinSlope(const Real rho_d, const Real Ni, const Real aU_i,
  ...
  auto Mj_t2 = (Ni / (4. * (aU_i - aL_i))) * (Kokkos::pow(x2 + adot_dt_this_i, 4.) -
                                              Kokkos::pow(x1 + adot_dt_this_i, 4.));

  auto Mj_fi_x2 = (Kokkos::pow(x2, 5.) / 5.) +
                  ((3. * adot_dt_this_i - aM_i) * Kokkos::pow(x2, 4.) / 4.) +
                  (adot_dt_this_i * (adot_dt_this_i - aM_i) * Kokkos::pow(x2, 3.)) +
                  (Kokkos::pow(adot_dt_this_i, 2.) * (adot_dt_this_i - 3 * aM_i) *
                   Kokkos::pow(x2, 2.) / 2.) -
...
// Implement eqn 34 of McKinnon+18 to calculate the number of grains to be contributed to
// a given single bin by another single bin during the update
KOKKOS_INLINE_FUNCTION
Real DustGetContributedNumberLinSlope(const Real Ni, const Real aU_i, const Real aL_i,
  ...
  auto Nj_t2 = Si * ((Kokkos::pow(x2, 2.) / 2.) - (aM_i * x2) -
                     (Kokkos::pow(x1, 2.) / 2.) + (aM_i * x1));
  auto contributed_number = Nj_t1 + Nj_t2;
  return contributed_number;
}

// Implement loglinear equivalent to eqn 37 of McKinnon+18 to calculate the mass to be
// contributed to a given single bin by another single bin during the update
KOKKOS_INLINE_FUNCTION
Real DustGetContributedMassLogLinSlope(const Real rho_d, const Real beta_i,
  ...
  if (std::abs(kappa_i) < 1000) {
    ...
    Real mj_fi_1 = StablePowDiff(
        x1, x2, kap_p_four); // Kokkos::pow(x2, kap_p_four) - Kokkos::pow(x1, kap_p_four);
    mj_fi_1 = mj_fi_1 / kap_p_four;

    Real mj_fi_2 = StablePowDiff(
        x1, x2, kap_p_thr); // Kokkos::pow(x2, kap_p_thr) - Kokkos::pow(x1, kap_p_thr);
    mj_fi_2 = 3. * adot_dt_this_i * mj_fi_2 / kap_p_thr;

    Real mj_fi_3 = StablePowDiff(
        x1, x2, kap_p_two); // Kokkos::pow(x2, kap_p_two) - Kokkos::pow(x1, kap_p_two);
    mj_fi_3 = 3. * Kokkos::pow(adot_dt_this_i, 2.) * mj_fi_3 / kap_p_two;

    Real mj_fi_4 = StablePowDiff(
        x1, x2, kap_p_one); // Kokkos::pow(x2, kap_p_one) - Kokkos::pow(x1, kap_p_one);
...
// Implement loglinear equivalent to eqn 34 of McKinnon+18 to calculate the number of
// grains to be contributed to a given single bin by another single bin during the update
KOKKOS_INLINE_FUNCTION
Real DustGetContributedNumberLogLinSlope(const Real beta_i, const Real kappa_i,
  ...
  // auto contributed_number = (beta_i/(kappa_i + 1.)) * (Kokkos::pow(x2, kappa_i+1.) -
  // Kokkos::pow(x1, kappa_i+1.));
  if (std::abs(kappa_i) < 1000) {
...
// Determine the contributed number and mass of grains from bin i to bin j for given
// composition during the grain size update step bins_overlap indicates whether grains in
// bin i can "reach"/contribute to bin j during this step, based on the adot calculated
// for bin i If so, the overlap is calculated and the mass and number contributions are
// determined The method used is determined by dust_piecewise_mode_int
...
// There is a ghost bin placed above the largest size bin, and the grains that end up
// there are re-distributed to the largest grain-size bin as described in McKinnon. Grains
// which go below the lowest tracked grain size are treated as destroyed.
KOKKOS_INLINE_FUNCTION
void DustGetMassAndNumberUpdates(
    ...
    const DustDevice &DustDevObj, const Real dt, const Real Mi_renorm_factor = 1) {
  ...
  if (I_ij == 1) {
    bins_overlap = 1;
    int index_into_Ni =
        dust_scalar_idx_start + (gc_i * 2 * dust_num_grains_sizes) + (2 * gs_i);
    ...
    if (dust_piecewise_mode_int == 1) {
      ...
      // if(cons(index_into_Mi, k, j, i) > 1e-100 && contributed_mass < 1e-100 &&
      // std::abs(kappa_i) < 1000 ){
      //   printf("cons(Mi, > 1e-20 && contributed_mass < 1e-20) rho_d=%e beta_i=%e
      //   kappa_i=%e x1=%e x2=%e adot_dt_this_i=%e contributed_mass = %e \n", rho_d,
      //   beta_i, kappa_i, x1, x2, adot_dt_this_i, contributed_mass);
      // }
    }

    if (ghost_bin) {
      // FJJ Add the excess mass to the largest tracked bin. Calculate the added number to
      // this bin by treating all the added mass as having the largest possible grain
      // radius, like in McKinnon
      Real single_grain_mass_at_upper_edge =
          (4. * Kokkos::numbers::pi / 3.) * rho_d *
          Kokkos::pow(aL_j, 3.); // aL_j for the ghost bin is the highest egde of the
...
void DustAddAGBWindContribution(
  ...
  // FJJ for PGrete - AGB wind injection still needs to be fully tested in-depth e.g. with
  // a one-zone model comparison, but initial tests looked good.
  const int dust_num_grains_sizes = DustDevObj.dust_num_grains_sizes;
  const int dust_scalar_idx_start = DustDevObj.dust_scalar_idx_start;
  const ParArray1D<Real> single_grain_densities = DustDevObj.single_grain_densities;
  const ParArray1D<Real> grainsize_bin_edges_microm =
  ...
  const Real dust_return_silicates_mass_fraction_per_megayear =
      DustDevObj.dust_return_silicates_mass_fraction_per_megayear;
  const Real dust_return_carbon_mass_fraction_per_megayear =
  ...
  const ParArray1D<Real> agb_normalised_carbonaceous_mass_distribution_array =
      DustDevObj.agb_normalised_carbonaceous_mass_distribution_array;
  const ParArray1D<Real> agb_normalised_carbonaceous_number_distribution_array =
      DustDevObj.agb_normalised_carbonaceous_number_distribution_array;
  const ParArray1D<Real> agb_normalised_silicate_mass_distribution_array =
      DustDevObj.agb_normalised_silicate_mass_distribution_array;
  const ParArray1D<Real> agb_normalised_silicate_number_distribution_array =
  ...
  if (DustDevObj.stellar_radial_profile == StellarRadialProfile::POWER_LAW) {
    M_star_this_cell = StellarDensityAtRadiusPowerLaw(r, gamma_star, stellar_mass_cent,
                                                      stellar_density_profile_r_low,
                                                      stellar_density_profile_r_up) *
                       volume;
  } else if (DustDevObj.stellar_radial_profile == StellarRadialProfile::PRUGNIELSIMIEN) {
    M_star_this_cell =
        PrugnielSimienStellarRhoProfile(
            r, DustDevObj.sersic_n, DustDevObj.sersic_Re, DustDevObj.stellar_profile_norm,
            stellar_density_profile_r_low, stellar_density_profile_r_up) *
        volume;
  } else {
  ...
  Real added_silicate_mass = dust_return_silicates_mass_fraction_per_megayear *
                             M_star_this_cell * dt * code_to_megayear;
  Real added_carbonaceous_mass = dust_return_carbon_mass_fraction_per_megayear *
                                 M_star_this_cell * dt * code_to_megayear;
      ...
      if (carbonaceous_grains == 1 && silicate_grains == 1) {
        if (gc_i == 0) {
          cons_pack(b, index_into_Mi, k, j, i) =
              cons_pack(b, index_into_Mi, k, j, i) +
              (added_carbonaceous_mass *
               agb_normalised_carbonaceous_mass_distribution_array[gs_i] / volume);
          cons_pack(b, index_into_Ni, k, j, i) =
              cons_pack(b, index_into_Ni, k, j, i) +
              (added_carbonaceous_mass *
               agb_normalised_carbonaceous_number_distribution_array[gs_i] / volume);
          total_mass_C += (added_carbonaceous_mass *
                           agb_normalised_carbonaceous_mass_distribution_array[gs_i]);
        } else if (gc_i == 1) {
          cons_pack(b, index_into_Mi, k, j, i) =
              cons_pack(b, index_into_Mi, k, j, i) +
              (added_silicate_mass *
               agb_normalised_silicate_mass_distribution_array[gs_i] / volume);
          cons_pack(b, index_into_Ni, k, j, i) =
              cons_pack(b, index_into_Ni, k, j, i) +
              (added_silicate_mass *
               agb_normalised_silicate_number_distribution_array[gs_i] / volume);
          total_mass_S += (added_silicate_mass *
                           agb_normalised_silicate_mass_distribution_array[gs_i]);
        }
        ...
        cons_pack(b, index_into_Mi, k, j, i) =
            cons_pack(b, index_into_Mi, k, j, i) +
            (added_silicate_mass * agb_normalised_silicate_mass_distribution_array[gs_i] /
             volume);
        cons_pack(b, index_into_Ni, k, j, i) =
            cons_pack(b, index_into_Ni, k, j, i) +
            (added_silicate_mass *
...
KOKKOS_INLINE_FUNCTION
void DustCalculateAGBWindContribution(
    Real &total_mass_C, Real &total_mass_S, Real &total_mass_over_whole_dist_and_comps,
    Real &stellar_mass_this_cell, const int b, const int k, const int j, const int i,
    const parthenon::MeshBlockPack<VariablePack<Real>> &cons_pack,
    const DustDevice &DustDevObj, const Real dt) {
  // FJJ for PGrete - AGB wind injection still needs to be fully tested in-depth e.g. with
  // a one-zone model comparison, but initial tests looked good.
    ...
    for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i++) {
      ...
      if (carbonaceous_grains == 1 && silicate_grains == 1) {
        if (gc_i == 0) {
          total_mass_C += (added_carbonaceous_mass *
                           agb_normalised_carbonaceous_mass_distribution_array[gs_i]);
          total_mass_over_whole_dist_and_comps +=
              (added_carbonaceous_mass *
               agb_normalised_carbonaceous_mass_distribution_array[gs_i]);
        } else if (gc_i == 1) {
          total_mass_S += (added_silicate_mass *
...
void GetUpdated_MjNj_ThisCompositionHelper(
    const Real internal_e, const int b, const int k, const int j, const int i,
    const int gc_i, const parthenon::MeshBlockPack<VariablePack<Real>> &cons_pack,
    const DustDevice &DustDevObj, const IndexRange &kb, const IndexRange &jb,
    const IndexRange &ib, const Real dt, const Real temperature, const View6DReal Mj_new,
    const View6DReal Nj_new, const View6DReal a_dot_view,
  ...
  for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i += 1) {
    ...
    PARTHENON_REQUIRE(a_dot_view(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) == 0.0,
                      "Contributing a_dot_view to dirty array!")
  ...
  for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i += 1) {
    for (int gs_j = 0; gs_j < dust_num_grains_sizes + 1; gs_j += 1) {
      ...
      DustGetMassAndNumberUpdates(bins_overlap, contributed_number, contributed_mass,
                                  gc_i, gs_i, gs_j, b, k, j, i, cons_pack, adot_this_i,
                                  DustDevObj, dt, Mi_renorm_factor);
      // printf("bins_overlap = %d, contributed_number = %g, contributed_mass = %g current
      // bin mass = %g current bin number = %g \n", bins_overlap, contributed_number,
      // contributed_mass, cons(index_into_Mi, k, j, i), cons(index_into_Ni, k, j, i));
      if (bins_overlap == 1) {
        // if Ghost bin, will rebin mass into final tracked bin
        if (gs_j == dust_num_grains_sizes) {
          Mj_new(gc_i, dust_num_grains_sizes - 1, b, k - kb.s, j - jb.s, i - ib.s) +=
              contributed_mass;
          Nj_new(gc_i, dust_num_grains_sizes - 1, b, k - kb.s, j - jb.s, i - ib.s) +=
              contributed_number;
        } else {
...
// not parallelised over gc_i and gs_i
KOKKOS_INLINE_FUNCTION
void DustDoUpdateStepEulerInCoolingSubcycle(
    const Real internal_e, const int b, const int k, const int j, const int i,
    const parthenon::MeshBlockPack<VariablePack<Real>> &cons_pack,
    const DustDevice &DustDevObj, const IndexRange &kb, const IndexRange &jb,
    const IndexRange &ib, const Real sub_dt, const View6DReal Mj_new,
    const View6DReal Nj_new, const View6DReal a_dot_view) {
  ...
  // internal_e is the original internal_e before updating after the current accepted
  // subcycle step
  const Real temperature = mbar_gm1_over_kb * internal_e;
  for (int gc_i = 0; gc_i < num_grain_compositions; gc_i++) {
    GetUpdated_MjNj_ThisCompositionHelper(internal_e, b, k, j, i, gc_i, cons_pack,
                                          DustDevObj, kb, jb, ib, sub_dt, temperature,
                                          Mj_new, Nj_new, a_dot_view);
  } // int gc_i = 0; gc_i < num_grain_compositions; gc_i ++)
}

// Do 1st order time integration for the dust integration
KOKKOS_INLINE_FUNCTION
void DustFilladotView(const Real temperature, const int gc_i, const int gs_i, const int b,
                      const int k, const int j, const int i,
                      const parthenon::MeshBlockPack<VariablePack<Real>> &cons_pack,
                      const DustDevice &DustDevObj, const IndexRange &kb,
                      const IndexRange &jb, const IndexRange &ib, const Real sub_dt,
                      const View6DReal Mj_new, const View6DReal Nj_new,
...
KOKKOS_INLINE_FUNCTION void
DustDoUpdateWithadotArray(const int gc_i, const int gs_i, const int gs_j, const int b,
                          const int k, const int j, const int i,
                          const parthenon::MeshBlockPack<VariablePack<Real>> &cons_pack,
                          const DustDevice &DustDevObj, const IndexRange &kb,
                          const IndexRange &jb, const IndexRange &ib, const Real dt,
                          ScatAcc Mj_new, ScatAcc Nj_new, const View6DReal a_dot_view) {
  const int dust_scalar_idx_start = DustDevObj.dust_scalar_idx_start;
  const int num_grain_compositions = DustDevObj.num_grain_compositions;
  const int dust_num_grains_sizes = DustDevObj.dust_num_grains_sizes;
  const Real mbar_gm1_over_kb = DustDevObj.mbar_gm1_over_kb;
  ...
  Real contributed_mass = 0.0;
  const Real Mi_renorm_factor = 1;
  DustGetMassAndNumberUpdates(bins_overlap, contributed_number, contributed_mass, gc_i,
                              gs_i, gs_j, b, k, j, i, cons_pack, adot_this_i, DustDevObj,
                              dt, Mi_renorm_factor);
  if (bins_overlap == 1) {
    // if Ghost bin, will rebin mass into final tracked bin
    if (gs_j == dust_num_grains_sizes) {
...
// Do 2nd order time integration for the dust integration, on a subcycling dt, so we are
// not parallelised over gc_i and gs_i
KOKKOS_INLINE_FUNCTION
void DustDoUpdateStepHeunsInCoolingSubcycle(
    ...
    const parthenon::MeshBlockPack<VariablePack<Real>> &cons_pack,
    const DustDevice &DustDevObj, const IndexRange &kb, const IndexRange &jb,
    const IndexRange &ib, const Real h, const View6DReal Mj_new, const View6DReal Nj_new,
    const View6DReal a_dot_view, const View5DReal heun_i, const View5DReal heun_ipeps,
  ...
  Real min_acceptable_dtg_ratio =
      1e-8 / dust_num_grains_sizes; // we cannot trust values under this, to set bins to
                                    // zero here to prevent issues with reconstruction

  for (int gc_i = 0; gc_i < num_grain_compositions; gc_i++) {
    // Must renormalise the mass and number to get things of sensible oom, to reduce
    // numerical error in the fluxes
    int Ni_norm_oom;
    int Mi_norm_oom;
    for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i++) {
      int index_into_Mi =
      ...
      if (gs_i == 0) {
        ...
        Ni_norm_oom = std::max(static_cast<int>(std::floor(
                                   std::log10(std::fabs(cons(index_into_Ni, k, j, i))))),
                               Ni_norm_oom);
        Mi_norm_oom = std::max(static_cast<int>(std::floor(
                                   std::log10(std::fabs(cons(index_into_Mi, k, j, i))))),
                               Mi_norm_oom);
      }
    }

    Real Mi_renorm_factor =
        Kokkos::pow(10., static_cast<int>(std::max(-100, Mi_norm_oom)));
    Real Ni_renorm_factor =
        Kokkos::pow(10., static_cast<int>(std::max(-100, Ni_norm_oom)));

    // FJJ: The mass renormalization has two components. First, we rescale the number
    // density and so the mass is rescaled in this way too, since Mi is proportional to mi
    // multiplied by a power term. Then, we should rescale again by Mi_renorm_factor_units
    // to get to a reasonable order of magntitude.
    //  we need Mi_renorm_factor_units to rescale the density in reconstruction functions
    //  etc
    ...
    for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i++) {
      ...
      Real dtg_ratio =
          cons(index_into_Mi, k, j, i) * Mi_renorm_factor / cons(IDN, k, j, i);
      if (dtg_ratio < min_acceptable_dtg_ratio) {
        cons(index_into_Mi, k, j, i) = 0.;
        cons(index_into_Ni, k, j, i) = 0.;
      }

      heun_i(index_into_heun_Ni, b, k - kb.s, j - jb.s, i - ib.s) =
          cons(index_into_Ni, k, j, i);
      heun_i(index_into_heun_Mi, b, k - kb.s, j - jb.s, i - ib.s) =
          cons(index_into_Mi, k, j, i);
    ...
    // We have 3 1st-order time integration steps we must do for the Heun method. Each
    // updates the mass and grain number distributions
    for (int hstep_i = 0; hstep_i < 3; hstep_i++) {
      ...
      GetUpdated_MjNj_ThisCompositionHelper(
          internal_e, b, k, j, i, gc_i, cons_pack, DustDevObj, kb, jb, ib, dt,
          temperature, Mj_new, Nj_new, a_dot_view, Mi_renorm_factor_units);

      for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i += 1) {
        int index_into_heun_Mi =
            (2 * gs_i) + 1;                  // heun arrays have no grain comp dimension
        int index_into_heun_Ni = (2 * gs_i); // heun arrays have no grain comp dimension
        int index_into_Mi =
            dust_scalar_idx_start + (2 * ((gc_i * dust_num_grains_sizes) + gs_i)) + 1;
        int index_into_Ni =
        ...
        Real new_Ni_dens =
            std::max(Nj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) / volume, 0.);
        Real new_Mi_dens =
            std::max(Mj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) / volume, 0.);

        if (new_Ni_dens / new_Ni_dens > 1e70 || new_Ni_dens / new_Ni_dens < 1e-70) {
          printf("normalised new_Ni and new_Mi differ massively!!! new_Ni_dens = %e "
                 "new_Ni_dens = %e \n",
        ...
        if (hstep_i == 0) {
          // we are at i + eps
          // store i+eps state
          heun_ipeps(index_into_heun_Ni, b, k - kb.s, j - jb.s, i - ib.s) =
              cons(index_into_Ni, k, j, i);
          heun_ipeps(index_into_heun_Mi, b, k - kb.s, j - jb.s, i - ib.s) =
              cons(index_into_Mi, k, j, i);
          ...
          // we are at i + 1
          // store i+1 state
          heun_ip1(index_into_heun_Ni, b, k - kb.s, j - jb.s, i - ib.s) =
              cons(index_into_Ni, k, j, i);
          heun_ip1(index_into_heun_Mi, b, k - kb.s, j - jb.s, i - ib.s) =
              cons(index_into_Mi, k, j, i);
        }
      }
    ...
    for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i += 1) {
      ...
      // Calculate the final distributions using predictor/corrector - Nj_new, Mj_new are
      // TOTAL number/mass (not densities)
      Nj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) =
          (s_i_N + 0.5 * h * (f_N0 + f_N1)) * volume * Ni_renorm_factor;
      Mj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) =
          (s_i_M + 0.5 * h * (f_M0 + f_M1)) * volume * Mi_renorm_factor;
      ...
      if (Nj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) < 0.) {
        // FJJ The euler update results in strictly positive mass and number final values.
        // The heun update does not however, since a large local gradient calculated on a
        // timestep << dt, then multiuplied by dt, does not gurarantee conservation. FJJ
        // TODO think about this more especially since the scheme used will then differ
        // between bins, and chat with PGrete

        // Real euler_gradient_N = (s_ip1_N - s_i_N) / h;
        // Real euler_gradient_M = (s_ip1_M - s_i_M) / h;
        // printf("Nj_new < 0! Nj_new = %e, euler_result = %e s_i_N=%e s_ipeps_N = %e
        // s_ip1peps_N =%e s_ip1_N=%e  ---- f_N0 =%e f_N1 = %e euler_gradient = %e
        // 0.5*(f_N0+f_N1)=%e. Will fall back to euler result \n", Nj_new(gc_i, gs_i, b, k
        // - kb.s, j - jb.s, i - ib.s)/Ni_renorm_factor, s_ip1_N, s_i_N, s_ipeps_N,
        // s_ip1peps_N, s_ip1_N, f_N0 , f_N1, euler_gradient_N, 0.5*(f_N0+f_N1) );
        printf("Nj_new < 0! Nj_new = %e, euler_result = %e orig_value=%e. Will fall back "
               "to euler result \n",
               Nj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) / Ni_renorm_factor,
               s_ip1_N, s_i_N);

        Nj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) =
            s_ip1_N * volume * Ni_renorm_factor;
        Mj_new(gc_i, gs_i, b, k - kb.s, j - jb.s, i - ib.s) =
            s_ip1_M * volume * Mi_renorm_factor;
      }

      // reset cons_pack to its original state
      cons(index_into_Ni, k, j, i) =
          heun_i(index_into_heun_Ni, b, k - kb.s, j - jb.s, i - ib.s) * Ni_renorm_factor;
      cons(index_into_Mi, k, j, i) =
          heun_i(index_into_heun_Mi, b, k - kb.s, j - jb.s, i - ib.s) * Mi_renorm_factor;

      if (cons(index_into_Ni, k, j, i) / cons(index_into_Mi, k, j, i) > 1e70 ||
          cons(index_into_Ni, k, j, i) / cons(index_into_Mi, k, j, i) < 1e-70) {
        printf("cons(index_into_Ni, k, j, i) and cons(index_into_Mi, k, j, i) differ "
               "massively!!! cons(index_into_Ni, k, j, i) = %e cons(index_into_Mi, k, j, "
               "i) = %e \n",
...
template <typename HostView, typename HostView_Rbin>
void Dust::WriteAGBInjectionHistory(
    const int num_r_bins_,
    const HostView host_reduction_view_agb_injected_mass_carbonaceous,
    const HostView host_reduction_view_agb_injected_mass_silicates,
    const HostView host_reduction_view_stellar_mass, const HostView_Rbin r_bin_edges,
    parthenon::MeshData<parthenon::Real> *md, const Real dt, const Real t) const {
  // FJJ TODO this function is hardwired for 2 compositions - probs will work okay if a
  // single comp type, but not more than 2
  auto hydro_pkg = md->GetBlockData(0)->GetBlockPointer()->packages.Get("Hydro");
  auto fluid = hydro_pkg->Param<Fluid>("fluid");
  const auto &cons_pack = md->PackVariables(std::vector<std::string>{"cons"});
  const auto units = hydro_pkg->Param<Units>("units");
  const auto dust_num_grain_compositions =
  ...
  MPI_Reduce(MPI_IN_PLACE, buf_C.data(), n, MPI_PARTHENON_REAL, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(MPI_IN_PLACE, buf_S.data(), n, MPI_PARTHENON_REAL, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(MPI_IN_PLACE, buf_stellarmass.data(), n, MPI_PARTHENON_REAL, MPI_SUM, 0,
             MPI_COMM_WORLD);
  if (my_rank == 0) {
    for (int i = 0; i < n; ++i) {
      // Unpack
    ...
    for (int gc_i = 0; gc_i < 3; gc_i++) {
      ...
      if (dust_file.tellp() == 0) {
        ...
        for (int i = 0; i < num_r_bins_; i++) { // radii bins
          dust_file << i << ":" << r_bin_edges[i] / kpc << "kpc"
                    << "-->" << r_bin_edges[i + 1] / kpc << "kpc"
                    << "|";
        }
      ...
      for (int i = 0; i < num_r_bins_; i++) { // radii bins
        ...
        if (gc_i == 0) {
          dust_file << " | "
                    << std::log10(host_reduction_view_agb_injected_mass_carbonaceous(i) /
                                  msun);
        } else if (gc_i == 1) {
          dust_file << " | "
                    << std::log10(host_reduction_view_agb_injected_mass_silicates(i) /
                                  msun);
        } else if (gc_i == 2) {
...
void GetMassChangeRatePerBin(const Real temperature, const Real rho, const Real volume,
  ...
  for (int gc_i = 0; gc_i < num_grain_compositions; gc_i++) {
    ...
    for (int gs_i = 0; gs_i < dust_num_grains_sizes; gs_i += 1) {
      ...
      // Check correct signs. Don;t worry too much if very near a boundary, where
      // densities might go weird
      if (adot_sputter > 0 || adot_sputter != adot_sputter) {
        if (std::abs(x) < 0.9 * whole_box_extent &&
            std::abs(y) < 0.9 * whole_box_extent &&
            std::abs(z) < 0.9 * whole_box_extent) {
          // printf("[FJJ DEBUG] Sputtering is growing grains! x=%e y =%e z=%e r=%e
          // whole_box_extent=%e f_sput=%e rho =%e  adot_sputter=%e  sput_prefac=%e
          // sput_dens=%e  sput_T=%e  \n", x,y,z,r, whole_box_extent, f_sput, rho,
          // adot_sputter,  sput_prefac,  sput_dens,  sput_T);
          printf("[FJJ DEBUG] Sputtering is growing grains in GetMassChangePerBin! x=%e "
                 "y =%e z=%e whole_box_extent=%e rho =%e\n",
                 x, y, z, whole_box_extent, rho);
        }
        if (std::abs(x) < 0.9 * whole_box_extent &&
            std::abs(y) < 0.9 * whole_box_extent &&
            std::abs(z) < 0.9 * whole_box_extent) { // ignore weird things at box boundary
                                                    // e.g. negative densities
          printf("[FJJ DEBUG] Sputtering is growing grains inside of the boundary in "
                 "GetMassChangePerBin! x=%e y =%e z=%e whole_box_extent=%e rho =%e "
                 "temperature=%e\n",
                 x, y, z, whole_box_extent, rho, temperature);
          PARTHENON_REQUIRE(adot_sputter <= 0,
                            "Sputtering is growing grains in GetMassChangePerBin!");
        }
        adot_sputter = 0.;
        adot_accretion = 0; // don;t do any dust updates if sputtering already is bad
        adot_total = 0.;
      ...
      if (adot_accretion < 0 || adot_accretion != adot_accretion) {
        if (std::abs(x) < 0.9 * whole_box_extent &&
            std::abs(y) < 0.9 * whole_box_extent &&
            std::abs(z) < 0.9 * whole_box_extent) {
          printf("[FJJ DEBUG] Accretion is shrinking grains in GetMassChangePerBin! x=%e "
                 "y =%e z=%e r=%e whole_box_extent=%e rho =%e  adot_accretion=%e  \n",
                 x, y, z, r, whole_box_extent, rho, adot_accretion);
        }
        ...
        if (std::abs(x) < 0.9 * whole_box_extent &&
            ...
            std::abs(z) < 0.9 * whole_box_extent) { // ignore weird things at box boundary
                                                    // e.g. negative densities
          printf("[FJJ DEBUG] Accretion is shrinking grains inside of the boundary in "
                 "GetMassChangePerBin! x=%e y =%e z=%e r=%e whole_box_extent=%e rho =%e  "
                 "adot_accretion=%e  \n",
                 x, y, z, r, whole_box_extent, rho, adot_accretion);
          PARTHENON_REQUIRE(adot_accretion >= 0,
                            "Accretion is shrinking grains in GetMassChangePerBin!");
        }
      ...
      if (dust_piecewise_mode_int == 2) {
        ...
        Real adot_to_mdot_prefactor =
            (Kokkos::pow(aU, kap_p_thr) - Kokkos::pow(aL, kap_p_thr)) *
            (4. * Kokkos::numbers::pi * rho_d * beta_i) / kap_p_thr;
