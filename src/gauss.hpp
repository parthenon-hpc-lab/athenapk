...
namespace parthenon::math::quadrature{ namespace detail{
...
public:
   ...
   template <class F>
   KOKKOS_FUNCTION static auto integrate(F f, Real* pL1 = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))
   {
   ...
   template <class F>
   KOKKOS_FUNCTION static auto integrate(F f, Real a, Real b, Real* pL1 = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))
   {
