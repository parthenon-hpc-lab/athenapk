...
// AthenaPK headers
#include "../main.hpp"
#include "few_modes_ft_lognormal.hpp"
...
FewModesFTLog::FewModesFTLog(parthenon::ParameterInput *pin, parthenon::StateDescriptor *pkg,
...
void FewModesFTLog::Generate(MeshData<Real> *md, const Real dt,
  ...
  pmb->par_for(
      ...
      KOKKOS_LAMBDA(const int n, const int m) {
        ...
        tmp = std::pow(kmag / kmin, -5./3.); // AMPLITUDE, TO BE REDEFINED TO LOGNORMAL DISTRIBUTION
        if (tmp < 0.) tmp = 0.;
...
// Creates a random set of wave vectors from k_min to k_max (limit imposed by cell size)
ParArray2D<Real> MakeRandomModesLog(const int num_modes, const Real k_min, const Real k_max,
  ...
  const int k_low = k_min;
  const int k_high = k_max;
