...
auto PrecipitatorProfile::ReadProfile(const std::string &filename) -> ProfileTuple {
  ...
  for (std::string line; std::getline(fstream, line);) {
    ...
    gravity_vec.push_back(values.at(3));
  ...
  PinnedArray1D<Real> gravity_("gravity", gravity_vec.size());
  ...
  for (int i = 0; i < static_cast<int>(z_vec.size()); ++i) {
    ...
    rho_(i) = rho_vec.at(i);
    P_(i) = P_vec.at(i);
    gravity_(i) = gravity_vec.at(i);
    phi_(i) = phi_vec.at(i);
    bfield_(i) = bfield_vec.at(i);
  }

  return std::make_tuple(z_, rho_, P_, gravity_, phi_, bfield_);
}

auto PrecipitatorProfile::GetZMin(const std::string &filename) -> Real {
  auto [z, rho, P, gravity, phi, bfield] = ReadProfile(filename);
...
auto PrecipitatorProfile::GetGravity(const std::string &filename) -> PinnedArray1D<Real> {
  auto [z, rho, P, gravity, phi, bfield] = ReadProfile(filename);
  return gravity;
}
...
PrecipitatorProfile::PrecipitatorProfile(const std::string &filename)
    : z_min_(GetZMin(filename)), z_max_(GetZMax(filename)), z_(GetZ(filename)),
      rho_(GetRho(filename)), P_(GetP(filename)), gravity_(GetGravity(filename)),
      phi_(GetPhi(filename)), bfield_(GetBField(filename)), spline_rho_(z_, rho_),
      spline_P_(z_, P_), spline_gravity_(z_, gravity_), spline_phi_(z_, phi_),
      spline_bfield_(z_, bfield_) {}
