...
class SphericalGravity {
 ...
 public:
  ...
  // "problem/cluster/gravity" for the cluster pgen's instantiation.
  SphericalGravity(parthenon::ParameterInput *pin, const std::string &block_prefix) {
    ...
    const parthenon::Real M_nfw_200 =
        pin->GetOrAddReal(block_prefix, "m_nfw_200", 8.5e14 * units.msun());
    ...
    const parthenon::Real M_bcg_s =
        pin->GetOrAddReal(block_prefix, "m_bcg_s", 7.5e10 * units.msun());
    ...
    const parthenon::Real m_smbh =
        pin->GetOrAddReal(block_prefix, "m_smbh", 3.4e8 * units.msun());
