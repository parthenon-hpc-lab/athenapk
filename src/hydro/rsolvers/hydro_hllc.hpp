...
struct Riemann<Fluid::euler, RiemannSolver::hllc> {
  static KOKKOS_INLINE_FUNCTION void Solve(const int k, const int j, const int i,
    ...
    wl(IDN, k, j, i) = sl * fl[IDN] + sr * fr[IDN];
    wl(ivx, k, j, i) = sl * fl[IV1] + sr * fr[IV1] + sm * cp;
    wl(ivy, k, j, i) = sl * fl[IV2] + sr * fr[IV2];
    wl(ivz, k, j, i) = sl * fl[IV3] + sr * fr[IV3];
    wl(IEN, k, j, i) = sl * fl[IEN] + sr * fr[IEN] + sm * cp * am;
  }
};
