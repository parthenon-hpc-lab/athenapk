...
TaskStatus CalculateFluxes(MeshData<Real> *u0_data, MeshData<Real> *u1_data,
  ...
  if (pmb->pmy_mesh->ndim >= 2) {
    ...
    // Testing team sizes and scratch
    ...
    for (scratch_level = 0; scratch_level < 2; scratch_level++) {
      ...
      for (int t = 0; t < 8; t++) {
        ...
        if (t < 7) {
          ...
          team_size =
              parthenon::team_policy(DevExecSpace(), NbNk, 1)
                  ...
                  .team_size_recommended(x2flux, Kokkos::ParallelForTag());
        }
        const int team_size_max =
            parthenon::team_policy(DevExecSpace(), NbNk, 1)
                .set_scratch_size(scratch_level, Kokkos::PerTeam(scratch_size_in_bytes))
                .team_size_max(x2flux, Kokkos::ParallelForTag());
        ...
        Kokkos::parallel_for("x2 flux" + suffix + " TVR scratch " +
                                 std::to_string(scratch_level) + " teamsize " +
                                 std::to_string(team_size),
                             policy.set_scratch_size(
                                 scratch_level, Kokkos::PerTeam(scratch_size_in_bytes)),
                             x2flux);
      }
    }
  }
  ...
  if (pmb->pmy_mesh->ndim >= 3) {
    ...
    for (int p = 0; p < 13; p++) {
      ...
      for (int t = 0; t < 8; t++) {
        ...
        Kokkos::parallel_for(
            "x3 flux" + suffix + " TVR better mem teamsize" + std::to_string(team_size) +
                " pencil " + std::to_string(pencil_width),
