...
TaskStatus CalculateFluxes(std::shared_ptr<MeshData<Real>> &md) {
  ...
  parthenon::par_for_outer(
      DEFAULT_OUTER_LOOP_PATTERN, "x1 flux inline Riemann", DevExecSpace(),
      scratch_size_in_bytes, scratch_level, 0, cons_in.GetDim(5) - 1, kl, ku, jl, ju,
      KOKKOS_LAMBDA(parthenon::team_mbr_t member, const int b, const int k, const int j) {
        const auto &prim = prim_in(b);
        auto &cons = cons_in(b);
        parthenon::ScratchPad2D<Real> wl(member.team_scratch(scratch_level),
        ...
        // Sync all threads in the team so that scratch memory is consistent
        member.team_barrier();
        constexpr int ivx = IV1;
        constexpr int ivy = IV1 + ((ivx - IV1) + 1) % 3;
        constexpr int ivz = IV1 + ((ivx - IV1) + 2) % 3;
  ...
  // auto flx_pack = md->PackVariables(std::vector<std::string>{"bnd_flux::cons"});
  for (int b = 0; b < prim_in.GetDim(5); b++) {
    auto mbd = md->GetBlockData(b);
    parthenon::ParArray4D<Real> x1flux = mbd->Get("bnd_flux::cons").data.Get(0, 0, 0);
    // auto desc = parthenon::MakePackDescriptor<var_t>(md.get(), {},
    // {PDOpt::WithFluxes}); auto pack = desc.GetPack(md.get());
    using TE = parthenon::TopologicalElement;
    parthenon::par_for_outer(
        DEFAULT_OUTER_LOOP_PATTERN, "x1 flux inline Riemann flxpack", DevExecSpace(),
        ...
        KOKKOS_LAMBDA(parthenon::team_mbr_t member, const int k, const int j) {
          ...
          parthenon::par_for_inner(member, ib.s, ib.e + 1, [&](const int i) {
            ...
            x1flux(IDN, k, j, i) = flxi[IDN];
            x1flux(ivx, k, j, i) = flxi[IV1];
            x1flux(ivy, k, j, i) = flxi[IV2];
            x1flux(ivz, k, j, i) = flxi[IV3];
            x1flux(IEN, k, j, i) = flxi[IEN];
            // flx_pack(b, TE::F1, IDN, k, j, i) = flxi[IDN];
            // flx_pack(b, TE::F1, ivx, k, j, i) = flxi[IV1];
            // flx_pack(b, TE::F1, ivy, k, j, i) = flxi[IV2];
            // flx_pack(b, TE::F1, ivz, k, j, i) = flxi[IV3];
            // flx_pack(b, TE::F1, IEN, k, j, i) = flxi[IEN];
          });
        });
  }
  //--------------------------------------------------------------------------------------
