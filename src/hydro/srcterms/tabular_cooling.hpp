...
class TabularCooling
{
  private:
    ...
    //TODO(forrestglines): Make log_lambdas_ explicitly a texture cache array, use CUDA to interpolate directly
    ...
    static KOKKOS_INLINE_FUNCTION parthenon::Real DeDt(
        const parthenon::Real& e,
        const parthenon::Real& mu_m_u_gm1_by_k_B, const parthenon::Real& n_h2_by_rho,
        const parthenon::Real& log_temp_start, const parthenon::Real& log_temp_final,
        const parthenon::Real& d_log_temp, const unsigned int n_temp,
      ...
      if( log_temp < log_temp_start){
        ...
        const unsigned int i_temp = static_cast<unsigned int>(
        ...
        log_lambda = log_lambda_i + (log_temp - log_temp_i)*(log_lambda_ip1 - log_lambda_i)/d_log_temp;
  ...
  public:
    ...
    void SubcyclingFirstOrderSrcTerm(parthenon::MeshData<parthenon::Real> *md, const parthenon::SimTime &tm) const;
