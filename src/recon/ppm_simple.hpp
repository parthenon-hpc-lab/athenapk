...
//========================================================================================
//! \brief piecewise parabolic reconstruction with two curvature limiter variants:
//!        - CS08 (McCorquodale/Colella) sign-based min-mod style limiter
//!        - van Albada symmetric limiter (smoother, reduces clipping at smooth extrema)
...
//  The symmetric form from van Albada, van Leer & Roberts, A&A, 108, 76 (1982):
//    f(a,b) = [a*(b^2 + eps^2) + b*(a^2 + eps^2)] / (a^2 + b^2 + 2*eps^2)
KOKKOS_INLINE_FUNCTION
Real VanAlbada(const Real a, const Real b) {
  const Real eps = 1.0e-14;
  const Real eps2 = eps * eps;
  const Real num = a * (b * b + eps2) + b * (a * a + eps2);
  const Real den = a * a + b * b + 2.0 * eps2;
...
//         Uses the symmetric van Albada limiter in the curvature-limiting step
//         (Step 2a) instead of the standard CS08 sign-based limiter.
//         Steps 1, 3, and 4 are identical to the standard PPM.
...
//! \fn Reconstruct<Reconstruction::ppm_va, int DIR>()
//  \brief Wrapper function for PPM van Albada reconstruction
//  In X1DIR call over [is-1,ie+1] to get BOTH L/R states over [is,ie]
//  In X2DIR call over [js-1,je+1] to get BOTH L/R states over [js,je]
//  In X3DIR call over [ks-1,ke+1] to get BOTH L/R states over [ks,ke]
template <Reconstruction recon, int XNDIR>
KOKKOS_INLINE_FUNCTION
    typename std::enable_if<recon == Reconstruction::ppm_va, void>::type
