...
AGNTriggering::AGNTriggering(parthenon::ParameterInput *pin,
  ...
  case AGNTriggeringMode::COLD_GAS: {
    ...
    // - ...reset...         : tells whether gas angular momentum quantities should be
    // reset to 0 at given timestep
    // - multistep_accretion : tells whether ongoing timestep is a the middle of an
    // ongoing accretion event
    // - accretion_counter   : indicates the number of timesteps since start of the
    // ongoing accretion event
    ...
    hydro_pkg->AddParam<bool>(
        "agn_triggering_reset_angular_momentum", false,
        Params::Mutability::Restart); // Boolean, tells whether the gas AM variables
                                      // should be reset
    hydro_pkg->AddParam<bool>(
        "multistep_accretion", false,
        Params::Mutability::Restart); // Boolean, check whether current accretion event
                                      // spans on multiple timesteps

    hydro_pkg->AddParam<int>("accretion_counter", 0,
                             Params::Mutability::Restart); // Accretion event timecounter
                                                           // (for multitimestep case)

    hydro_pkg->AddParam<Real>(
        "eddington_ratio", 0.0,
        Params::Mutability::Restart); // Time of last accretion event
    hydro_pkg->AddParam<Real>(
        "t_acc_last", 0.0, Params::Mutability::Restart); // Time of last accretion event
    hydro_pkg->AddParam<Real>(
        "dt_acc", 0.0,
        Params::Mutability::Restart); // Duration of the ongoing accretion event
    ...
    hydro_pkg->AddParam<Real>("J_BH_z",
                              pin->GetReal("problem/cluster/agn_feedback", "a_BH") *
                                  units.gravitational_constant() *
                                  (mass_smbh * mass_smbh) / units.speed_of_light(),
                              Params::Mutability::Restart);
    hydro_pkg->AddParam<Real>("a_BH",
                              pin->GetReal("problem/cluster/agn_feedback", "a_BH"),
                              Params::Mutability::Restart);
...
//===============================================================================================
template <typename EOS>
void AGNTriggering::ReduceAngularMomentum(parthenon::Real &J_gas_x,
                                          parthenon::Real &J_gas_y,
  ...
  if (init_spin_BH) { // Useless in theory since already called in
    ...
    Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i,
          ...
          // Only calculate angular momentum of gas inside the sphere of radius
          // J_gas_radius
        ...
        Kokkos::Sum<Real>(md_J_gas_x), Kokkos::Sum<Real>(md_J_gas_y),
        Kokkos::Sum<Real>(md_J_gas_z));
...
parthenon::TaskStatus AGNTriggeringUpdateBH(parthenon::MeshData<parthenon::Real> *md,
  ...
  case AGNTriggeringMode::COLD_GAS: {
    ...
    hydro_pkg->UpdateParam("J_gas_x_cumulative",
                           hydro_pkg->Param<parthenon::Real>("J_gas_x_cumulative") +
                               hydro_pkg->Param<parthenon::Real>("J_gas_x"));
    hydro_pkg->UpdateParam("J_gas_y_cumulative",
                           hydro_pkg->Param<parthenon::Real>("J_gas_y_cumulative") +
                               hydro_pkg->Param<parthenon::Real>("J_gas_y"));
    hydro_pkg->UpdateParam("J_gas_z_cumulative",
                           hydro_pkg->Param<parthenon::Real>("J_gas_z_cumulative") +
                               hydro_pkg->Param<parthenon::Real>("J_gas_z"));

    if (init_spin_BH) {

      hydro_pkg->UpdateParam("accretion_counter",
                             hydro_pkg->Param<int>("accretion_counter") + 1);
      ...
      const int accretion_counter = hydro_pkg->Param<int>(
          "accretion_counter"); // Counter. 0 if t_acc <= dt, >= 1 otherwise.
      const Real t_acc_last =
          hydro_pkg->Param<Real>("t_acc_last"); // Time of the last accretion event
      const Real dt_acc =
          hydro_pkg->Param<Real>("dt_acc"); // Duration of the current accretion event
      const Real a_BH = hydro_pkg->Param<Real>(
          "a_BH"); // Spin of the BH at previous timestep (to calculate t_acc)
      ...
      if (eddington_ratio >= 0.01) {
        ...
        if (hydro_pkg->Param<bool>("multistep_accretion")) {
          if (tm.time - t_acc_last > dt_acc) {
            ...
            hydro_pkg->UpdateParam(
                "t_acc_last",
                tm.time); // Updating the time of the last (ie. current accretion time)
            hydro_pkg->UpdateParam("multistep_accretion", false);
            hydro_pkg->UpdateParam("agn_triggering_reset_angular_momentum", true);
          } else {
            update_spin =
                false; // Accretion event still ungoing, not updating the BH spin yet.
          }
        } else {
          // Calculate the new accretion event duration and store it into hydro_pkg
          const Real dt_acc_new = 0.003 * std::pow(a_BH, 7.0 / 8.0) *
                                  std::pow(mass_smbh_8, 11.0 / 8.0) *
                                  std::pow(eddington_ratio, -3.0 / 4.0);
          ...
          if (dt_acc_new > tm.dt) {
            ...
            update_spin = true;
            hydro_pkg->UpdateParam("t_acc_last",
                                   tm.time); // Update time of last accretion event
            hydro_pkg->UpdateParam("multistep_accretion", false);
            hydro_pkg->UpdateParam("agn_triggering_reset_angular_momentum", true);
          }
        }
        ...
        const Real J_BH_magnitude =
            std::sqrt(J_BH_x * J_BH_x + J_BH_y * J_BH_y + J_BH_z * J_BH_z);

        // Compute the specific spin of the black hole
        const Real a_BH = J_BH_magnitude * units.speed_of_light() /
                          (units.gravitational_constant() *
                           std::pow(hydro_pkg->Param<Real>("mass_smbh"), 2.0));
        ...
        const Real J_gas_x = hydro_pkg->Param<Real>("J_gas_x_cumulative") /
                             hydro_pkg->Param<int>("accretion_counter");
        const Real J_gas_y = hydro_pkg->Param<Real>("J_gas_y_cumulative") /
                             hydro_pkg->Param<int>("accretion_counter");
        const Real J_gas_z = hydro_pkg->Param<Real>("J_gas_z_cumulative") /
                             hydro_pkg->Param<int>("accretion_counter");

        // Calculate the magnitude of the angular momentum vector
        const Real J_gas_magnitude =
            std::sqrt(J_gas_x * J_gas_x + J_gas_y * J_gas_y + J_gas_z * J_gas_z);
        ...
        const Real J_d_over_2J_bh =
            6.8e-2 * std::pow(eddington_ratio / (radiative_efficiency / 0.1), 1.0 / 8.0) *
            std::pow(mass_smbh_8, 23.0 / 16.0) * std::pow(a_BH, 3.0 / 16.0);

        // Compare with cos(theta)
        const Real cos_theta = (J_gas_x * J_BH_x + J_gas_y * J_BH_y + J_gas_z * J_BH_z) /
                               (J_gas_magnitude * J_BH_magnitude);
        ...
        const Real J_tot_magnitude =
            std::sqrt(J_tot_x * J_tot_x + J_tot_y * J_tot_y + J_tot_z * J_tot_z);
        ...
        const Real new_a_BH = J_tot_magnitude * units.speed_of_light() /
                              (units.gravitational_constant() *
                               std::pow(hydro_pkg->Param<Real>("mass_smbh"), 2.0));

        /*
        if (parthenon::Globals::my_rank == 0) {
          std::cout << "multistep_accretion =  " <<
        hydro_pkg->Param<bool>("multistep_accretion") << std::endl; std::cout <<
        "accretion_counter   =  " << hydro_pkg->Param<int>("accretion_counter")    <<
        std::endl; std::cout << "dt_acc              =  " <<
        hydro_pkg->Param<Real>("dt_acc")              << std::endl; std::cout << "dt_hydro
        =  " << tm.dt                                         << std::endl; std::cout <<
        "dt_ratio            =  " << hydro_pkg->Param<Real>("dt_acc")/ tm.dt       <<
        std::endl; std::cout << "t_acc_last          =  " <<
        hydro_pkg->Param<Real>("t_acc_last")          << std::endl; std::cout <<
        "unclipped new_a_BH  =  " << new_a_BH        << std::endl;
        }
        */
        ...
        if (update_spin) {
          ...
          hydro_pkg->UpdateParam(
              "theta_BH", std::acos(hydro_pkg->Param<Real>("J_BH_z") / J_tot_magnitude));
          hydro_pkg->UpdateParam("phi_BH", std::atan2(hydro_pkg->Param<Real>("J_BH_y"),
                                                      hydro_pkg->Param<Real>("J_BH_x")));
...
AGNTriggeringMPIReduceTriggering(parthenon::StateDescriptor *hydro_pkg) {
  ...
  case AGNTriggeringMode::COLD_GAS: {
    ...
    PARTHENON_MPI_CHECK(MPI_Allreduce(MPI_IN_PLACE, &angular_gas_x, 1, MPI_PARTHENON_REAL,
                                      MPI_SUM, MPI_COMM_WORLD));
    hydro_pkg->UpdateParam("J_gas_x", angular_gas_x);

    // y-component
    Real angular_gas_y = hydro_pkg->Param<Real>("J_gas_y");
    PARTHENON_MPI_CHECK(MPI_Allreduce(MPI_IN_PLACE, &angular_gas_y, 1, MPI_PARTHENON_REAL,
                                      MPI_SUM, MPI_COMM_WORLD));
    hydro_pkg->UpdateParam("J_gas_y", angular_gas_y);

    // z-component
    Real angular_gas_z = hydro_pkg->Param<Real>("J_gas_z");
    PARTHENON_MPI_CHECK(MPI_Allreduce(MPI_IN_PLACE, &angular_gas_z, 1, MPI_PARTHENON_REAL,
                                      MPI_SUM, MPI_COMM_WORLD));
