...
void MagneticTower::MagneticFieldSrcTerm(
  ...
  if( hydro_pkg->Param<Fluid>("fluid") != Fluid::glmmhd ){
    PARTHENON_FAIL("MagneticTower::MagneticFieldSrTerm: Non-divergence cleaning, cell centered magnetic field methods not supported");
  }
  ...
  parthenon::par_for(
    ...
    KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i){
      ...
      cons(IEN, k, j, i) += prim(IB1, k, j, i)*b_x + prim(IB2, k, j, i)*b_y + prim(IB3, k, j, i)*b_z 
                            + 0.5*(b_x*b_x + b_y*b_y + b_z*b_z) ;
                          
      //Update the magnetic fields
      //We're just using cell centered fields here and not bothering with a vector potential
      //Hopefully divB will get removed during divergence cleaning.
      cons(IB1, k, j, i) += b_x;
      cons(IB2, k, j, i) += b_y;
      cons(IB3, k, j, i) += b_z;
    });
...
struct ReductionSumArray{
  ...
  ReductionSumArray<ScalarType,N>& operator += (const ReductionSumArray<ScalarType,N>& src) {
    for ( int i = 0; i < N; i++ ) {
      data[i]+=src.data[i];
    }
    return *this;
  ...
  void operator += (const volatile ReductionSumArray<ScalarType,N>& src) volatile {
    for ( int i = 0; i < N; i++ ) {
      data[i]+=src.data[i];
    }
  }
...
void MagneticTower::ReducePowerContrib(parthenon::MeshData<parthenon::Real> *md) const {
  ...
  hydro_pkg->UpdateParam("mt_linear_contrib",
    linear_contrib + hydro_pkg->Param<parthenon::Real>("mt_linear_contrib"));
  hydro_pkg->UpdateParam("mt_quadratic_contrib",
    quadratic_contrib + hydro_pkg->Param<parthenon::Real>("mt_quadratic_contrib"));
...
void InitInitialMagneticTower(std::shared_ptr<StateDescriptor> hydro_pkg, parthenon::ParameterInput *pin){
  ...
  MagneticTower initial_magnetic_tower(
    initial_magnetic_tower_field,initial_magnetic_tower_alpha,initial_magnetic_tower_l_scale,
    initial_jet_coords);
...
void InitFeedbackMagneticTower(std::shared_ptr<StateDescriptor> hydro_pkg, parthenon::ParameterInput *pin){
  ...
  MagneticTower feedback_magnetic_tower(
    feedback_strength,feedback_magnetic_tower_alpha,feedback_magnetic_tower_l_scale,
    feedback_jet_coords);
