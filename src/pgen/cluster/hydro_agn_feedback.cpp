...
HydroAGNFeedback::HydroAGNFeedback(parthenon::ParameterInput* pin):
  ...
  jet_height_(pin->GetReal("problem/cluster","agn_jet_height")),
  jet_coords_(pin)
  {}
                                       ...
                                       const parthenon::SimTime &tm) const
{
  ...
  const Real thermal_feedback = thermal_power*thermal_scaling_factor*beta_dt;//energy/volume
  ...
  const Real kinetic_feedback =  kinetic_power*kinetic_scaling_factor*beta_dt;//energy/volume

  //Note that new mass is injected to create the kinetic power, separate from the existing gas
  const Real jet_total_mass = kinetic_power/(
    ...
    KOKKOS_LAMBDA(const int &b, const int &k, const int &j, const int &i){
      ...
      if(kinetic_power > 0 ){
        ...
        if(r < jet_radius && fabs(h) < jet_height){
          ...
          jet_coords.jet_vector_to_cartesian(time,
            r,cos_theta,sin_theta,h,
            0,0,1,//Unit vector pointing along jet axis
            jet_x, jet_y, jet_z);

          const int sign_jet = (h > 0)? 1 : -1;//Above or below jet-disk

          cons(IDN,k,j,i) += jet_density*beta_dt;//mass/volume
          cons(IM1,k,j,i) += jet_density*sign_jet*jet_x*jet_velocity*beta_dt;//velocity*mass/volume
          cons(IM2,k,j,i) += jet_density*sign_jet*jet_y*jet_velocity*beta_dt;//velocity*mass/volume
          cons(IM3,k,j,i) += jet_density*sign_jet*jet_z*jet_velocity*beta_dt;//velocity*mass/volume
