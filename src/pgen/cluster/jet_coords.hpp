 ...
 public:
  ...
  SimCartToJetCylCoords(const parthenon::Real x_sim, const parthenon::Real y_sim,
    ...
    const parthenon::Real x_jet = x_sim * cos_phi_jet_axis_ * cos_theta_jet_axis_ +
                                  y_sim * sin_phi_jet_axis_ -
                                  z_sim * sin_theta_jet_axis_ * cos_phi_jet_axis_;
    const parthenon::Real y_jet = -x_sim * sin_phi_jet_axis_ * cos_theta_jet_axis_ +
                                  y_sim * cos_phi_jet_axis_ +
                                  z_sim * sin_phi_jet_axis_ * sin_theta_jet_axis_;
    const parthenon::Real z_jet =
        x_sim * sin_theta_jet_axis_ + z_sim * cos_theta_jet_axis_;
  ...
  KOKKOS_INLINE_FUNCTION void JetCylToSimCartVector(
    ...
    v_x_sim = v_x_jet * cos_phi_jet_axis_ * cos_theta_jet_axis_ -
              v_y_jet * sin_phi_jet_axis_ * cos_theta_jet_axis_ +
              v_z_jet * sin_theta_jet_axis_;
    v_y_sim = v_x_jet * sin_phi_jet_axis_ + v_y_jet * cos_phi_jet_axis_;
    v_z_sim = -v_x_jet * sin_theta_jet_axis_ * cos_phi_jet_axis_ +
              v_y_jet * sin_phi_jet_axis_ * sin_theta_jet_axis_ +
              v_z_jet * cos_theta_jet_axis_;
  }
};
/************************************************************
