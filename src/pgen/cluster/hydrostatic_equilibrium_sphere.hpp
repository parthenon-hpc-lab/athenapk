...
class HydrostaticEquilibriumSphere
{
  private:
    ...
    parthenon::Real P_from_rho_K (const parthenon::Real rho, const parthenon::Real K) const {
      const parthenon::Real P = K*pow(mu_/mu_e_,2./3.)*pow(rho/(mu_*atomic_mass_unit_),5./3.);
      return P;
    }
    ...
    parthenon::Real rho_from_P_K (const parthenon::Real P, const parthenon::Real K) const {
      const parthenon::Real rho = pow(P/K,3./5.)*mu_*atomic_mass_unit_/pow(mu_/mu_e_,2./5);
      return rho;
    }
    ...
    class dP_dr_from_r_P_functor {
      ...
      public:
        dP_dr_from_r_P_functor (const HydrostaticEquilibriumSphere<GravitationalField,EntropyProfile> &sphere):
          sphere_(sphere)
      {}
    ...
    parthenon::Real step_rk4( const parthenon::Real t0, const parthenon::Real t1, const parthenon::Real y0, Function f) const{
  ...
  public:
    ...
    class PRhoProfile{
      ...
      public:
        PRhoProfile(const View1D R, const View1D P, const HydrostaticEquilibriumSphere &sphere):
          R_(R),P_(P),sphere_(sphere),
          n_R_(R_.extent(0)),R_start_(R_(0)),R_end_(R_(n_R_-1))
        {}
    ...
    PRhoProfile<View1D> generate_P_rho_profile(
        parthenon::IndexRange ib, parthenon::IndexRange jb, parthenon::IndexRange kb,
        Coords coords) const;

    template <typename View1D>
    PRhoProfile<View1D> generate_P_rho_profile(
        const parthenon::Real R_start, const parthenon::Real R_end, const unsigned int n_R) const;
